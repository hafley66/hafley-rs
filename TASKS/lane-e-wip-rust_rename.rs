...
//! granularity only; an owner segment (`Owner { .. }`, `Kind::Old`) is proved
//! through the anchor module or a module-scope named `use`, never a block one.
...
impl Rename for RustSource {
    ...
    fn respell_symbol(
        ...
        // A shorthand key `Owner { old }` respells to `new: old` so the local
        // binding keeps its name. The typing walk proves the shape, so a
        // use-tree leaf spelled the same way never reads as a key.
...
impl Corpus {
    ...
    fn harvest(
        ...
        let home = self.home(rel);
        let anchor_module = anchored.map(|decl| module_of(self.home(&request.anchor), &decl.chain));
        ...
        for leaf in &scan.uses {
            ...
            let binds_variant = anchored.is_some_and(|decl| match &decl.kind {
                DeclKind::Variant { owner } => self.variant_leaf(
                    home,
                    &leaf.chain,
                    &leaf.prefix,
                    owner,
                    anchor_module.as_ref().expect("a variant anchor has a module"),
                ),
                _ => false,
            });
            match leaf.kind {
        ...
        for path in &scan.paths {
            if !path.prefix.is_empty() {
                let variant = anchored.is_some_and(|decl| match &decl.kind {
        ...
        if let Some(DeclKind::Field { owner }) = anchored.map(|decl| &decl.kind) {
                    ...
                    FieldSite::Owner { span, chain, prefix, owner: site_owner, pattern, .. }
                        if site_owner == owner
                            && self.owner_reach(
                                ...
                                anchor_module
                                    .as_ref()
                                    .expect("a field anchor has a module"),
                                nameable,
                                scan,
                            ) =>
                    {
    ...
    /// Whether a `use` leaf's prefix ends at the anchor enum from its module:
    /// `use Kind::Old;` carries a variant where the segments before the owner
    /// resolve to the enum's module.
    fn variant_leaf(
        &self,
        home: &ModuleId,
        chain: &[String],
    ...
    /// Whether a path's owner segment names the anchor's owner in this scope:
    /// the segments before it resolve to the anchor module, or a module-scope
    /// `use` binds the owner's name from it.
    #[allow(clippy::too_many_arguments)]
    fn owner_reach(
        &self,
        home: &ModuleId,
...
/// The same-file typing walk behind field seats, mirroring the receiver
/// plane's rules (`rust_receivers.rs:9`): a param annotation, `let x: T`,
/// `self`, a struct field's declared type, one hop through a same-file
/// return. Everything else stays unknown, and unknown is a stop.
struct FieldWalk<'a> {
    old: &'a str,
    line_starts: &'a [u32],
...
/// The same-file type tables, the receiver plane's `tables` restated: returns
/// by fn name, associated returns by (self type, method), fields by (struct,
/// field), through inline mods.
fn field_tables(
    items: &[syn::Item],
...
/// The directory a non-`#[path]` `mod x;` reads its file from: the declaring
/// file's directory unless that file is a crate root or a `mod.rs`, which own
/// it directly, and then `<own dir>/<stem>` for a plain module file.
...
/// The files a `#[path = ".."]` decl names, to the module each is. Rustc reads
/// the literal against the declaring file's module directory, so an override
/// can point anywhere in the crate; a file two decls name is a stop, one seat
/// per attr literal.
