...
fn listed_paths(repository: &Repository, revision: &RevisionId, pathspecs: &[String]) -> Result<Vec<String>> {
    let mut command = Command::new("git");
    command.arg("-C").arg(&repository.root).args(["ls-files", "-z"]);
    ...
    Ok(paths
        ...
        .filter_map(|(path, answer)| {
            ...
            match fields.as_slice() {
                [oid, "blob", size] => entry_with_size(repository, revision, path, oid, size.parse().ok()),
                _ => None,
            }
        })
        .collect())
