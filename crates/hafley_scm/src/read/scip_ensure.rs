...
//!
//! THE RUN ITSELF is delegated to the `ScipSource` impls in `crate::read::scip`, not
...
use crate::read::types::{ScipError, ScipSource};
...
pub static INDEXERS: &[Indexer] = &[
    Indexer {
        ...
        source: &crate::read::scip::ScipRust,
    },
    Indexer {
        lang: "typescript",
        ...
        source: &crate::read::scip::ScipTypescript,
    },
    Indexer {
        lang: "python",
        ...
        source: &crate::read::scip::ScipPython,
    },
    Indexer {
        lang: "go",
        markers: &["go.mod"],
    ...
    Indexer {
        ...
        source: &crate::read::scip::ScipJava,
    },
    Indexer {
        lang: "cpp",
        ...
        source: &crate::read::scip::ScipClang,
    },
];
...
impl IndexSet {
        ...
        I: IntoIterator<Item = (String, String)>,
    {
        ...
        let joined: String = entries
            ...
            .collect();
        let digest = match crate::read::shape::ContentId::blake3(joined.as_bytes()) {
            crate::read::shape::ContentId::Blake3(bytes) => hex_of(&bytes),
            crate::read::shape::ContentId::GitBlob(oid) => oid.0.to_string(),
        };
        Self { entries, digest }
    }
...
fn content_digest(bytes: &[u8]) -> String {
    match crate::read::shape::ContentId::blake3(bytes) {
        crate::read::shape::ContentId::Blake3(digest) => hex_of(&digest),
        crate::read::shape::ContentId::GitBlob(oid) => oid.0.to_string(),
    }
}
...
pub fn ensure_index_picked_for_root(
    root: &Path,
    cache_dir: &Path,
    budget: IndexBudget,
    pick: IndexerPick,
...
fn place(
    ...
    if let [(_, only)] = parts {
        ...
        crate::read::scip_decode::merge_indexes(&sources, &out)?;
    }
...
pub fn root_key(root: &Path) -> String {
    match crate::read::shape::ContentId::blake3(root.as_os_str().as_encoded_bytes()) {
        crate::read::shape::ContentId::Blake3(bytes) => hex_of(&bytes[..8]),
        crate::read::shape::ContentId::GitBlob(oid) => oid.0.to_string(),
    }
}
