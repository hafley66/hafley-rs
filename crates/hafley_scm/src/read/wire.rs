...
//!
//! The envelope types (`SpanOut`, `FlatFact`) now live in `crate::read::types`; this
...
//! `crate::read::scip_rows` on size and on subject: this module flattens the four
...
use crate::read::shape::{content_id_of, Strings};
use crate::read::source::RyiOutput;
use crate::read::tsi::types::{
    Arg as TsiArg, CoverageOut, FactOut, Method, RunOut, WitnessOut, PROTOCOL_VERSION,
};
use crate::read::types::{CfgF, DataF};
pub use crate::read::types::{FlatFact, SpanOut};
...
pub fn flatten_each<E>(
    ...
    let span = crate::read::trace::phase_span("-", crate::read::trace::Phase::Flatten);
    ...
    crate::read::trace::record_phase(&span, 0, facts, 1);
    outcome?;
    if let Some(run) = witness {
...
fn flatten_call<E>(
    ...
    for node in &bundle.nodes {
        // The module-as-caller def (call_kinds MODULE_CALLER) is a
        ...
        // are untouched.
        if node.kind == crate::read::lang::call_kinds::MODULE_CALLER {
            continue;
        }
        push(FlatFact::Node {
            fact: None,
...
/// `crate::read::cfg`, never projected by a `Source`, so nothing carries it here.
