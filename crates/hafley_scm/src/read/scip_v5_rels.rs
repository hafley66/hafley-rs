...
//! unchanged.
//!
//! HOW THIS RELATES TO `crate::read::scip_rows`. That module is PASSTHROUGH: every
...
use crate::read::scip::{byte_range_at, LineTable};
use crate::read::types::{FlatFact, OccurrenceRole, ScipIndex, ScipOccurrence};
...
pub fn usable_symbol(symbol: &str) -> bool {
...
pub fn is_callable_def(symbol: &str) -> bool {
    symbol.ends_with(").")
}
...
fn is_module_binding_reference(
    document: &crate::read::types::ScipDocument,
