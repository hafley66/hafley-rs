...
pub fn go_parse_shared(content: &str) -> Option<std::sync::Arc<tree_sitter::Tree>> {
...
pub fn go_parse_shared_keyed(
    content: &str,
) -> Option<(ContentId, std::sync::Arc<tree_sitter::Tree>)> {
    use crate::read::shape::content_id_of;
    thread_local! {
        static LAST: std::cell::RefCell<Option<(crate::read::shape::ContentId, std::sync::Arc<tree_sitter::Tree>)>> =
            const { std::cell::RefCell::new(None) };
    }
    let id = crate::read::dispatch::extracting_blob(content.as_bytes())
...
/// `pub`: `go_modules.rs`'s own dedicated parse reuses this walk
/// directly rather than re-deriving the kind/name/module table a second time.
pub fn go_walk_import_specs(
    node: tree_sitter::Node,
    src: &[u8],
    strings: &mut Strings,
...
/// `go.mod`, with that file's `module` line. `pub`: `go_modules.rs`
...
pub fn go_package_dir(module: &GoModule, import_path: &str) -> Option<PathBuf> {
...
pub fn go_publish_file_facts(path: &str, blob: Option<&ContentId>, facts: Arc<GoFileFacts>) {
...
pub fn go_is_method_def(path: &str, span: Span) -> bool {
...
pub fn go_file_facts_of_source(tree: &tree_sitter::Tree, src: &str) -> GoFileFacts {
