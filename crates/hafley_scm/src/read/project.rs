...
//! (`crate::read::wire::FlatFact`), with flat top-level path and name fields on the
...
impl LegTrail {
    ...
    fn push<F: crate::read::family::Family>(&mut self, edge: &ProjectEdge<F>) {
        if self.on {
...
fn resolve_project_inputs(
    ...
    let mut macro_rows: Vec<Vec<crate::read::lang::rust_scip_macros::MacroSiteRow>> = Vec::new();
    if request.arms.call {
        let macro_files: Vec<crate::read::lang::rust_scip_macros::ScipMacroFile> = inputs
            .iter()
            .map(|input| crate::read::lang::rust_scip_macros::ScipMacroFile {
                path: &input.path,
                blob: &input.blob,
                ...
                .map(|((input, (_, edges)), rows)| {
                    ...
                    for row in rows {
                        out.push(FlatFact::MacroSiteOut {
                            family: crate::read::shape::FamilyTag::Call,
                            span: crate::read::wire::SpanOut::new(row.span.start, row.span.end()),
    ...
    if let Some(last) = inputs.last() {
        if request.arms.call || request.arms.types {
            crate::read::types::set_own(Some(last.blob.clone()));
        }
    }
...
pub fn fill_indexes(
    ...
    cx.indexes
        .kinds
        .set(stage_span("index_kind").in_scope(|| crate::read::types::build_kind_index(pairs)))
        .expect("fresh project kind index");
    cx.indexes
        .paths
        .set(crate::read::types::build_path_index(
            inputs
                .iter()
...
fn tsi_relations(rows: &[crate::read::tsi::FactOut]) -> Vec<String> {
...
fn envelope(input: Envelope) -> Vec<FlatFact> {
    ...
    for mut fact in facts {
        ...
        if let FlatFact::Fact(row) = &mut fact {
            ...
            witnesses.push(WitnessOut {
                fact: numbered,
                run: SYNTAX_RUN,
                method: crate::read::tsi::Method::Parse,
            });
            rows.push(fact);
            continue;
        }
    ...
    if let Some((_, run)) = semantic.iter().find(|(lang, _)| *lang == "ts") {
        if let Some(index) = cx.indexes.ts_checker.get() {
            ids = crate::read::tsi::emit_semantic(run.run, index, ids, &mut rows);
        }
    }
    ...
    if let Some((_, run)) = semantic.iter().find(|(lang, _)| *lang == "rust") {
        if let Some(index) = cx.indexes.rust_checker.get() {
            crate::read::tsi::emit_semantic(run.run, index, ids, &mut rows);
            ...
            for path in &index.unmodulated {
                rows.push(FlatFact::Diagnostic(crate::read::tsi::DiagnosticOut {
                    run: run.run,
...
fn load_ts_checker(
    ...
    let index = crate::read::lang::ts_checker::TsCheckerIndex::build(
        answers,
        corpus,
        cx.indexes.def_index.get().expect("the def index is set"),
    );
...
fn load_go_checker(
    ...
    let answers = match crate::read::lang::go_checker::answer(&root, &files, cx.witness) {
        Ok(answers) => answers,
        Err(err) => {
    ...
    let index = crate::read::lang::go_checker::GoCheckerIndex::build(
        answers,
        corpus,
        cx.indexes.def_index.get().expect("the def index is set"),
    );
...
pub fn scip_facts(request: &ResolveRequest) -> Result<Vec<FlatFact>, ProjectError> {
    ...
    Ok(crate::read::scip_rows::flatten_scip_records(
        &index,
        &reader,
...
pub fn scip_file_edges(request: &ResolveRequest) -> Result<Vec<FlatFact>, ProjectError> {
    ...
    Ok(crate::read::wire::scip_file_edges(&index))
}
...
pub struct ScipFamilyRequest<'a> {
    ...
    pub indexer: crate::read::scip_ensure::IndexerPick<'a>,
}
...
fn scip_family_from_path(
    ...
    facts.extend(crate::read::scip_v5_rels::v5_rel_rows(
        &index,
        request.root,
        &slug,
    ));
...
fn module_facts_of(path: &str, content: &[u8], wanted: bool) -> Option<ModuleFacts> {
    wanted.then(|| crate::read::lang::ts_resolve::module_facts(path, content))?
}
...
fn go_module_facts_of(path: &str, content: &[u8], wanted: bool) -> Option<GoModuleFacts> {
    wanted.then(|| crate::read::lang::go_modules::go_module_facts(path, content))?
}
...
pub fn index_set_of(inputs: &[ProjectInput]) -> crate::read::scip_ensure::IndexSet {
    crate::read::scip_ensure::IndexSet::new(
        inputs
            .iter()
...
pub static RESOLVE_ARMS: &[ResolveArm] = &[
    ResolveArm {
        ...
        drops: Some(crate::read::lang::ts::call_drops),
    ...
    ResolveArm {
        ...
        drops: Some(crate::read::lang::rust::call_drops),
        type_plane: TypePlane::Nodes,
    },
    ResolveArm {
        name: "go",
        ...
        drops: Some(crate::read::lang::go::call_drops),
        type_plane: TypePlane::Nodes,
    },
    ResolveArm {
        name: "kotlin",
        ...
        drops: Some(crate::read::lang::kotlin::call_drops),
        type_plane: TypePlane::Nodes,
    },
    ResolveArm {
        name: "prolog",
    ...
    ResolveArm {
        ...
        types: None,
        drops: Some(crate::read::lang::fallback::call_drops),
        type_plane: TypePlane::Nodes,
    },
];
...
fn resolve_call_edges(path: &str, output: &RyiOutput, cx: &ProjectCx) -> Vec<ProjectEdge<CallF>> {
    ...
    let leg = crate::read::trace::phase_span(arm.name, crate::read::trace::Phase::ResolveLeg);
    ...
    crate::read::trace::record_phase(&leg, 0, edges.len() as u64, asked as u64);
    edges
}
...
pub fn import_facts(input: &ProjectInput, cx: &ProjectCx) -> Vec<FlatFact> {
...
fn call_drop_facts(
    ...
    drops(&input.output, cx, edges)
        ...
        .map(|drop| FlatFact::Unresolved {
            family: crate::read::shape::FamilyTag::Call,
            path: Some(input.path.clone()),
            span: crate::read::wire::SpanOut::new(drop.span.start, drop.span.end()),
...
pub fn scip_conformances(
    ...
    let joined = crate::read::scip::join_documents(index, reader);
    ...
    for (doc_ix, document) in index.documents.iter().enumerate() {
        ...
        let lines = crate::read::scip::LineTable::build(content);
        for info in &document.symbols {
            if !info.relationships.iter().any(|rel| rel.is_implementation) {
                continue;
            }
            let symbol = index.symbol(info.symbol);
            if !crate::read::scip_v5_rels::usable_symbol(symbol) {
                continue;
            }
            let Some(owner_name) = crate::read::scip_v5_rels::descriptor_name(symbol) else {
                continue;
            };
                ...
                .filter(|rel| rel.is_implementation)
            {
                ...
                if !crate::read::scip_v5_rels::usable_symbol(target) {
                    continue;
                }
                let Some(target_name) = crate::read::scip_v5_rels::descriptor_name(target) else {
                    continue;
                };
                let Some((target_doc, _)) =
                    crate::read::scip::definition_of(index, doc_ix, related.symbol)
                else {
                    continue;
                };
                ...
                out.push(ScipConformance {
                    ...
                    target_path,
                    target_name,
                    method: crate::read::scip_v5_rels::is_callable_def(symbol)
                        || crate::read::scip_v5_rels::is_callable_def(target),
                });
            }
        }
    }
...
pub fn conformance_edges(inputs: &[ProjectInput], rows: &[ScipConformance]) -> Vec<FlatFact> {
    rows.iter()
...
fn conformance_tsi_rows(
    ...
    base: u32,
) -> (Vec<crate::read::tsi::FactOut>, u32) {
    ...
    for (input_ix, input) in inputs.iter().enumerate() {
        ...
        let mut sink = crate::read::tsi::TsiSink::new(0, crate::read::tsi::Method::Parse);
                ...
                None => {
                    ...
                    sink.fact(
                        ...
                        vec![
                            ...
                            crate::read::types::span_arg(row.owner_span),
                        ],
                    );
        ...
        let facts: Vec<crate::read::tsi::FactOut> = sink
            .rows()
            .into_iter()
...
fn caller_name(
    ...
    if node.kind == crate::read::lang::python::MODULE_CALLER {
        return None;
    }
