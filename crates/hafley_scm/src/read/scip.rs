...
//! trait + diet types live in `crate::read::types`, re-exported through
//! `crate::read::seams`; this module is the `wire.rs`-style logic half).
//!
...
//! vendored `proto/scip.proto`); they stay private to `crate::read::scip_decode`,
//! which owns the protobuf -> flat-types decode. Only the types in
//! `crate::read::types` cross the seam.
...
use crate::read::scip_decode::load_index;
...
fn persistent_stage(bin: &str, root: &Path) -> Result<PathBuf, ScipError> {
    let key = crate::read::scip_ensure::root_key(root);
...
pub fn build_doc_spans(doc: &ScipDocument, content: &[u8]) -> DocSpans {
...
pub fn build_def_map(index: &ScipIndex) -> DefMap {
...
) -> Vec<Option<(crate::read::shape::ContentId, Vec<u8>)>> {
    index
        .documents
        .iter()
        .map(|doc| {
            reader(&doc.relative_path)
                .map(|content| (crate::read::shape::content_id_of(&content), content))
        })
        .collect()
}
