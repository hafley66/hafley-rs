...
//! under `~/.omp/agent/sessions/--<encoded cwd>--/<timestamp>_<id>.jsonl`; the
//! transcript readers land in a later lane, so this skeleton lists no sessions
//! and reads no bytes.
...
/// (or no `--mode` at all) leaves the interactive frontend in charge.
const HEADLESS_MODES: [&str; 4] = ["json", "rpc", "acp", "rpc-ui"];
...
impl Harness for Omp {
    ...
    /// omp runs its TUI unless the invocation names a headless output mode:
    /// `--print`/`-p` one-shots, or `--mode json|rpc|acp|rpc-ui`.
...
/// omp's session root: `$PI_CONFIG_DIR` when set, else `~/.omp`, under
/// `agent/sessions`.
fn omp_sessions_dir() -> Result<PathBuf> {
    let root = match std::env::var_os("PI_CONFIG_DIR").filter(|value| !value.is_empty()) {
        Some(dir) => PathBuf::from(dir),
        None => dirs::home_dir()
            .ok_or_else(|| anyhow::anyhow!("resolve omp config dir"))?
            .join(".omp"),
    };
    Ok(root.join("agent").join("sessions"))
}
