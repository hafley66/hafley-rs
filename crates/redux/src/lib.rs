...
//!
//! - The reducer is intended to be a pure function `(&State, &Action) -> State`. Same actions in,
//!   same state out, on every machine, every replay. No I/O, no clock, no globals.
...
//!
//! The public surface is split into [`Store`] for live subscribers,
//! [`replay`] for action-log folds, [`Slice`] for composable reducers, and
//! [`Pool`] for fixed-capacity snapshot-owned storage.
//!
//! ## Quick start
//!
//! A reducer can feed both a live [`Store`] and a deterministic [`replay`]
//! check. Subscribers observe committed states after each dispatch.
//!
//! ```
//! use redux::{replay, Store};
//!
//! #[derive(Clone, Copy, Debug, PartialEq)]
//! struct Score(u32);
//! #[derive(Clone, Copy)]
//! enum Action { Add(u32), Reset }
//! fn reduce(state: &Score, action: &Action) -> Score {
//!     match action { Action::Add(points) => Score(state.0 + points), Action::Reset => Score(0) }
//! }
//!
//! let mut store = Store::new(reduce, Score(0));
//! let seen = std::rc::Rc::new(std::cell::RefCell::new(Vec::new()));
//! let sink = seen.clone();
//! let subscription = store.subscribe(move |state| sink.borrow_mut().push(state.0));
//! store.dispatch(Action::Add(3));
//! store.dispatch(Action::Add(4));
//! assert_eq!(*store.state(), Score(7));
//! assert_eq!(&*seen.borrow(), &[3, 7]);
//! store.unsubscribe(subscription);
//! assert_eq!(replay(reduce, Score(0), [Action::Add(3), Action::Add(4)]), Score(7));
//! ```
...
pub use _3_machine::{Edge, Graph, GraphError, Node};
pub use effects::reduce_then_apply;
...
impl<S, A> Store<S, A> {
    /// Create a store with a reducer and initial state.
    ...
    /// Detach a subscription; unknown or already-detached handles are ignored.
