//! Generic reducer algebra: one `(state, event, context) -> output` shape, reusable for any
//! partition of a larger system.
//!
//! The shape is a convention, not an enforced boundary. `reduce` receives caller-owned `State` and
//! a synchronous effect sink, and returns its `Output`. An implementer can still perform I/O,
//! mutate external state, or emit effects that the caller must settle; the trait neither inspects
//! effect values nor requires that all mutable state live in `State`. Compositions are zero-sized
//! types, which lets pipelines monomorphize.

use core::marker::PhantomData;

/// One reducer step. `Output` is the value handed to the next slice in the same dispatch; `Effect`
/// is a zero-or-more channel the sink receives synchronously inside `reduce`, with settlement
/// timing (immediate or deferred) chosen by the caller.
pub trait Slice {
    type Context<'a>: Copy;
    type State;
    type Event;
