...
pub const AFTER_HELP: &str = concat!(
    ...
    "\n",
    "Commands:\n",
    "  extract diff ROOT --from REV --to REV [--pattern GLOB]... [--sqlite PATH]\n",
    "                          [--json]\n",
    "    The blob delta between two commits of the repository rooted at ROOT. Both\n",
    "    revisions are resolved to full shas with `git rev-parse` and named in the\n",
    "    `diff_run` header. Every blob's bytes are read with `git cat-file`, never\n",
    "    from the checkout, so a dirty worktree changes nothing.\n",
    "    Rows are JSONL on stdout, sorted by (record, path): the `diff_run` header\n",
    "    then one `diff_file` row per added, removed or digest-changed path.\n",
