...
struct Relocation {
    ...
    old_path: Vec<String>,
    new_path: Vec<String>,
    /// Pre-move rels. Every edit stages ahead of the Moves (`0_move.rs:235-247`),
    /// so a parent this same batch creates is edited at the path it still wears.
    old_parent: String,
    new_parent: String,
...
/// zero-length span is an insertion, which soopy plans (`_7d_mutation_plan.rs:828`).
struct RelocateEdit {
    importer: String,
    span: Span,
    text: String,
...
/// into ONE insertion: `0_move.rs:267` gives an offset a single claimant.
fn insert_decls(
    cx: &MoveCx,
    moves: &BTreeMap<String, Relocation>,
