...
use crate::scip::{byte_range_at, join_documents, LineTable};
use crate::scip_ensure::{default_cache_dir, ensure_index_picked_for_root, IndexBudget};
use crate::scip_v5_rels::descriptor_name;
use crate::seams::{FileSet, IndexBag, ManifestMap, ProjectCx, ProjectDigest};
use crate::shape::{ContentId, FamilyTag, Span};
use crate::types::{covering_def, OccurrenceRole, ResolutionOrigin, ScipIndex, SymbolId};
use crate::wire::{FlatFact, SpanOut};
...
pub fn slow_project_with_raw<E>(
    ...
    let index = match index {
        Some(path) => crate::scip_decode::load_index(path)
        ...
        None => {
            ...
            crate::scip_decode::load_index(&path)
                .map_err(|error| ResolveWithRawError::Project(ProjectError::Scip(error)))?
        }
    };
...
fn owner_name(
    types: &crate::rows::FamilyBundle<crate::types::TypeF>,
    strings: &crate::shape::Strings,
    owner: Span,
) -> Option<String> {
...
fn checker_facts(files: &[PathBuf], root: &Path) -> Result<Vec<FlatFact>, ProjectError> {
    let present: BTreeSet<&str> = files
        .iter()
        .filter_map(|path| crate::lang::source_for(&path.to_string_lossy()).map(|src| src.name()))
        .collect();
    ...
    let request = crate::project::ResolveRequest {
        paths: files,
        arms: crate::project::ResolveArms { call: true, types: true, flow: false },
        scip: crate::project::ScipMode::Off,
        project_root: None,
        scip_records: crate::scip_rows::ScipRecords::all(),
    ...
    Ok(crate::project::resolve_project(&request)?
        .into_iter()
