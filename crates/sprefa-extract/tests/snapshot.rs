...
fn roster_routes_by_extension() {
    ...
    assert_eq!(source_for("x.md").expect(".md").name(), "markdown");
    assert_eq!(
        source_for("x.markdown").expect(".markdown").name(),
        "markdown"
    );
    // No roster `Source` claims the `.dl` extension.
    assert!(source_for("x.dl").is_none());
