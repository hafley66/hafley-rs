//! Lane C, rust: the receiver leg fires on a SPELLED receiver, and the
//! binding-typed legs (field, constructor return, trait-bound generic) bind
//! through the same corpus impl table. C.1 records a spelled path receiver
//! (`CstProjector.project(..)`) as `Named` instead of `Inferred`; C.2 lets
//! `recv_t` bind it to the corpus impl; C.5 exercises the three data-backed
//! legs. C.6: a plain call to a scope-bound name (`fn run(project: impl Fn())
//! { project() }`) is shadowed, so it emits no edge to a same-named free fn
//! and drops with reason `inferred`.
//!
//! Fixtures: `tests/fixtures/rust/spelled_receiver/src/`.
...
fn has(rows: &[(String, String, String, String)], caller: &str, callee: &str, file: &str) -> bool {
    rows.iter().any(|(c, n, f, _)| c == caller && n == callee && f == file)
}
