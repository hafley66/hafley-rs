...
/// because the no-flag default is per language (family.rs
/// `default_keeps_cst`); parity needs the same mask on both sides, and
/// `cst,type,call,df,data` is `FamilyMask::ALL` spelled out.
#[test]
