...
typed inserts and parameterized multi-row statements, generated by
...
checks ordinal overflow, groups rows by table while preserving their original
ordinals, and chunks each table's inserts against the connection's variable
and SQL-length limits. It validates capacities for all present tables before
writing. It does not create or commit transactions. Individual
...
The CLI derives its row-buffer ceiling from SQLite's runtime limits and also
tracks an 8 MiB serialized-input budget. It flushes before adding a fact that
would cross that byte budget, on reaching either ceiling, on source changes,
on explicit flush, and at completion. A fact larger than the byte budget is
accepted and flushed alone. Serialized-byte accounting does not bound exact
heap usage. Each table chunk uses one multi-row `INSERT ... VALUES` execution.
Prepared statement cache capacity equals the number of generated tables;
different chunk sizes can occupy different cache entries. Table keys are single
