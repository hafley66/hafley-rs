...
fn main() -> Result<()> {
    ...
    let needs_startup_sync = startup_sync_wanted(&cli.command, sync_suppressed());
...
mod tests {
    ...
    fn the_no_sync_hatch_clears_every_verb_that_would_sync() {
        ...
        for argv in syncing {
            let cli = Cli::try_parse_from(&argv).unwrap_or_else(|e| panic!("{argv:?}: {e}"));
            assert!(
                startup_sync_wanted(&cli.command, false),
                "{argv:?} syncs by default"
            );
            assert!(
                !startup_sync_wanted(&cli.command, true),
    ...
    fn the_no_sync_hatch_never_makes_a_registry_verb_sync() {
        ...
        assert!(!startup_sync_wanted(&cli.command, false));
        assert!(!startup_sync_wanted(&cli.command, true));
    }
