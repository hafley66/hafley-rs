...
/// Epoch-ms of the newest row touching each route, from one mailbox pass. The
/// stale alarm's own row is skipped: it is the symptom, not activity, or the
/// flag would clear itself each time it fired.
