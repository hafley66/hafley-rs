...
fn claude_reader_resolves_exact_session() -> Result<()> {
    ...
    if std::env::var_os("BOOP_E2E_READER_CHILD").is_some() {
        ...
        let messages = adapter.messages(&session, None);
            ...
            messages
                ...
                .collect::<Vec<_>>()
                == vec![("assistant", "fixture-answer")]
        );
        ensure!(
            adapter.native_settings(&session)
                == Some(boop::harness::NativeTuiEvent::Settings {
                    session_id: "owned-reader".into(),
    ...
    let result = bounded(
        Command::new(std::env::current_exe()?)
            .args([
                "--exact",
                "t4_lifecycle_gate::claude_reader_resolves_exact_session",
                "--nocapture",
            ])
            .env("BOOP_E2E_READER_CHILD", "1")
            .env("BOOP_READER_HOME", &root),
    );
...
fn native_registry_process_transition_preserves_trace() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("boop-registry-transition-{}", std::process::id()));
    std::fs::create_dir(&root)?;
...
"#,
    ...
    let spec = boop::harness::NativeTuiSpec {
        executable: env!("CARGO_BIN_EXE_boop").into(),
        cwd: root.clone(),
        args: Vec::new(),
        env: Vec::new(),
    };
    ...
    owner.frontend = Some(
        Command::new(&spec.executable)
            .args(["tui", "claude", "--name", "owned", "--bin"])
            .arg(&native)
            .arg("--cwd")
            .arg(&root)
            .boop_test_root(&root)
            .env("BOOP_CLAUDE_SESSIONS_DIR", &sessions)
            .env("BOOP_NO_SYNC", "1")
        ...
        if let Some(route) = bus::routes_in(&store)?.get("owned") {
            if route.session_id.as_deref() == Some("second") {
                let pid = store
                    ...
                    .context("no second process observation")?;
            ...
            "same-process registry transition was not bound"
        );
        std::thread::sleep(Duration::from_millis(100));
    };
    owner.stop();
    ensure!(
        !boop::live::pid_alive(pid as u32),
        "fixture native process survived owner cleanup"
    );
    Ok(())
}
...
trait LifecycleHarness {
    ...
    fn exit(&self, fixture: &Fixture) -> Result<()> {
        self.control(fixture, "/exit")
    }
    fn automatic_restart(&self) -> bool {
        false
    }
    fn approve_completion(&self, _fixture: &Fixture, _command: &str) -> Result<()> {
        Ok(())
    }
    ...
    fn busy(&self, fixture: &Fixture, registry: &Registry) -> Result<bool> {
        ...
        Ok(self
            ...
            .is_some_and(|session| {
                Some(&session.session_id) == route.session_id.as_ref()
    ...
    fn observe(
        ...
        registry: &Registry,
        route: &bus::Route,
    ) -> Result<Vec<boop_harness::transcript::Message>> {
    ...
    fn adapter<'a>(&self, registry: &'a Registry) -> &'a dyn Harness {
    ...
    fn rpc(&self, fixture: &Fixture, method: &str, params: Value) -> Result<Value> {
        ...
        for (id, method, params) in [
            (
                1,
                "initialize",
                json!({"clientInfo":{"name":"boop-lifecycle-gate","version":"1"},"capabilities":{"experimentalApi":true}}),
            ),
            (2, method, params),
        ] {
            ...
            loop {
                    ...
                    Instant::now() < deadline,
                    "Codex {method} deadline exceeded"
                );
...
impl LifecycleHarness for Codex {
    fn backend(&self, fixture: &Fixture) -> Result<Option<(u32, bool)>> {
        let socket = fixture
            .route()?
            .app_server_socket
            .context("owned Codex backend address absent")?;
    ...
    fn compact_count(&self, registry: &Registry, route: &bus::Route) -> Result<usize> {
        Ok(native_records(self.adapter(registry), route)?
            .iter()
    ...
    fn busy(&self, fixture: &Fixture, _registry: &Registry) -> Result<bool> {
        let result = self.rpc(
            fixture,
            "thread/read",
            json!({"threadId":fixture.route()?.session_id,"includeTurns":false}),
        )?;
        Ok(result
    ...
    fn approve_completion(&self, fixture: &Fixture, command: &str) -> Result<()> {
        ...
        loop {
            ...
            if screen.contains("This command requires approval") {
                    ...
                    compact(&screen).contains(&compact(command)),
                    "approval prompt differs from the authorized test-owned completion command"
                );
                std::fs::write(fixture.root.join("completion-approval.txt"), screen)?;
                fixture.tmux(&["send-keys", "-t", &fixture.pane, "Enter"])?;
                return Ok(());
            }
            if bus::messages_in(&fixture.store()?)?.iter().any(|message| {
                message.from == fixture.route
                    && message.to == fixture.parent
                    && message.kind == "result"
            }) {
                return Ok(());
            }
                ...
                Instant::now() < deadline,
                "completion command neither executed nor requested its scoped approval"
            );
            std::thread::sleep(Duration::from_millis(250));
        }
    }
    ...
    fn backend(&self, fixture: &Fixture) -> Result<Option<(u32, bool)>> {
        let address = url::Url::parse(
            &fixture
                .route()?
                .app_server_socket
                .context("owned OpenCode backend address absent")?,
        )?;
        let port = address.port().context("owned OpenCode port absent")?;
        Ok(Some((
            fixture.owned_command_pid(&format!("serve --port {port}"))?,
            false,
        )))
    }
    fn entry(&self) -> &str {
...
impl Fixture {
    ...
    fn store(&self) -> Result<Store> {
        Store::open(self.store_root.join("boop.db"))
    }
    fn route(&self) -> Result<bus::Route> {
        bus::routes_in(&self.store()?)?
            .remove(&self.route)
            .context("wrapper route not observed")
    }
    fn boop(&self, args: &[&str]) -> Result<String> {
        ...
        bounded(
            Command::new(&self.binary)
                ...
                .env_remove("BOOP_READER_HOME")
                .env("BOOP_DOOR_FLOOR", "30")
                .env("BOOP_NO_SYNC", "1")
                .env("BOOP_DB", self.store_root.join("boop.db"))
    ...
    fn launch(&self, harness: &dyn LifecycleHarness, resume: Option<&str>) -> Result<()> {
        ...
        for (key, value) in [
            ...
            (
                "BOOP_DB",
                self.store_root.join("boop.db").display().to_string(),
            ),
            (
                "BOOP_MAIL_DIR",
                self.store_root.join("mail").display().to_string(),
            ),
            (
                "BOOP_CONFIG",
                self.root.join("config.json").display().to_string(),
            ),
            ("BOOP_SESSION", self.parent.clone()),
            ("BOOP_LANE", self.parent.clone()),
        ] {
            script.push_str(&format!("export {key}={}\n", shell_quote(&value)));
        }
        script.push_str("eval \"$(boop shell-init bash)\"\n");
        script.push_str(&shell_quote(harness.entry()));
        for arg in harness.launch_args(resume) {
            script.push(' ');
        ...
        let command = format!(
            "timeout -s TERM -k 10s 240s bash {}",
            shell_quote(&launch.display().to_string())
        );
        let hash = bounded(Command::new("shasum").args(["-a", "256"]).arg(binary))?;
        std::fs::write(
            self.root.join(format!("{index}_launch.json")),
    ...
    fn cleanup(&self) -> Result<Value> {
        if self.cleaned.get() {
            return Ok(json!({"already_cleaned":true}));
        }
        ...
        let deadline = Instant::now() + Duration::from_secs(12);
        loop {
            let alive: Vec<_> = owned
                .iter()
                .copied()
                .filter(|pid| boop::live::pid_alive(*pid))
                .collect();
            if alive.is_empty() {
                return Ok(json!({"owned_pids":owned,"surviving_pids":alive}));
            }
            ensure!(
                Instant::now() < deadline,
                "test-owned processes survived tmux cleanup: {alive:?}"
            );
            std::thread::sleep(Duration::from_millis(250));
        }
    }
    fn owned_pids(&self) -> Result<std::collections::BTreeSet<u32>> {
        let pid: u32 = self
            .tmux(&["display-message", "-p", "-t", &self.pane, "#{pane_pid}"])?
            .trim()
            .parse()?;
        let processes = bounded(Command::new("ps").args(["-axo", "pid=,ppid="]))?;
        let processes: Vec<(u32, u32)> = processes
            .lines()
            .filter_map(|line| {
        ...
        loop {
            let children: Vec<_> = processes
                .iter()
                .filter(|(_, parent)| owned.contains(parent))
                .map(|(pid, _)| *pid)
                .collect();
    ...
    fn owned_command_pid(&self, needle: &str) -> Result<u32> {
        ...
        let matches: Vec<u32> = output
            .lines()
            .filter(|line| line.contains(needle))
            .filter_map(|line| line.split_whitespace().next()?.parse().ok())
            .collect();
        ensure!(
            matches.len() == 1 && owned.contains(&matches[0]),
            "backend selector did not identify one test-owned process"
        );
        Ok(matches[0])
    }
}
    ...
    loop {
        if let Ok(route) = fixture.route() {
            if route.session_id.is_some() {
                let live = fixture
                    .store()?
                    .live_row(route.session_id.as_deref().unwrap())?;
                if !live
                    .and_then(|row| row.pid)
                    .is_some_and(|pid| boop::live::pid_alive(pid as u32))
                {
                    ensure!(
                        fixture
                            .tmux(&["display-message", "-p", "-t", &fixture.pane, "#{pane_dead}"])?
                            .trim()
                            != "1",
                        "native frontend exited before live identity was established"
                    );
                    std::thread::sleep(Duration::from_millis(250));
                    ensure!(
                        Instant::now() < deadline,
                        "no live frontend identity within 35 seconds"
                    );
                    continue;
                }
            ...
            Instant::now() < deadline,
            "no observed wrapper identity within 35 seconds"
        );
        std::thread::sleep(Duration::from_millis(250));
    }
}
...
fn inbound_receipts_exclude_instructions_quoting_the_body() {
    ...
    assert_eq!(
        [
            ...
            "When the message \"lane owned done rc=0\" arrives, reply ACK",
        ]
        .map(|text| incoming_body_matches(text, body)),
        [true, true, false]
    );
}
    ...
    loop {
        ...
        if let Ok(rows) = harness.observe(registry, &route) {
            let users = rows
                .iter()
                .filter(|row| {
                    matches!(row.role.as_str(), "user" | "meta")
                        && incoming_body_matches(&row.text, body)
                })
                .count();
            let answers = rows
                .iter()
                .filter(|row| {
                    row.role == "assistant" && row.text.lines().any(|line| line.trim() == answer)
                })
                .count();
            ensure!(
                users <= 1 && answers <= 1,
                "duplicate native receipt: {users} user, {answers} assistant"
            );
            if users == 1 && answers == 1 {
                let screen =
                    fixture.tmux(&["capture-pane", "-p", "-t", &fixture.pane, "-S", "-100"])?;
                ...
                let message = bus::messages_in(&store)?
                    ...
                    .context("missing sent envelope")?;
                ensure!(store.delivery_accepted(&message.id, &fixture.route)?);
                let routes = bus::routes_in(&store)?;
                for _ in 0..2 {
                    ensure!(
                        boop::mail::deliver_hail(registry, &store, &routes, &message)?.rung
                            == boop::mail::Rung::AlreadyAccepted
                    );
                }
                ...
                let after = harness.observe(registry, &route)?;
                    ...
                    after
                        .iter()
                        .filter(|row| matches!(row.role.as_str(), "user" | "meta")
                            && incoming_body_matches(&row.text, body))
                        .count()
                        == users
                );
                    ...
                    after
                        ...
                        .filter(|row| row.role == "assistant"
                            && row.text.lines().any(|line| line.trim() == answer))
                        .count()
                        == answers
                );
                let session = harness
                ...
                let boop::harness::NativeTuiEvent::Settings {
                    ...
                    model.is_some() && current.model == model,
                    "route model differs from native execution: {:?} vs {:?}",
                    current.model,
                    model
                );
                ensure!(
                    store.session_attr(&session_id, "effort")? == effort,
                    "stored effort differs from native execution"
                );
                let pid = store
                    .live_row(&session_id)?
                    .and_then(|row| row.pid)
                    .context("receipt has no native PID")?;
                ensure!(
                    boop::live::pid_alive(pid as u32),
                    "receipt PID is no longer live"
                );
                ensure!(
                    current.parent.as_deref() == Some(fixture.parent.as_str())
                        && current.tmux.as_deref() == Some(fixture.pane.as_str()),
                    "receipt route ownership changed"
                );
                let pane_receipt = fixture.root.join(format!("receipt-{}.txt", message.id));
                std::fs::write(&pane_receipt, screen)?;
                return Ok(
                    json!({"message_id":message.id,"thread":route.session_id,"user_count":users,"answer_count":answers,"send":sent,"model":model,"effort":effort,
        ...
        if Instant::now() >= deadline {
            ...
            if let Some(error) = pane
                .lines()
                .find(|line| line.contains("subscription plan") && line.contains("include access"))
            {
                anyhow::bail!(
                    "BLOCKED: provider rejected current subscription access: {}",
                    error.trim()
                );
            }
...
fn resume(
    ...
    let trace = fixture
        .store()?
        .trace_of(thread)?
        .context("native session has no Boop trace")?;
    let old_pid = fixture
        .store()?
        .live_row(thread)?
        .and_then(|row| row.pid)
        .context("native session has no observed PID")?;
        ...
        != "1"
    {
            ...
            Instant::now() < deadline,
            "native exit control did not finish within 15 seconds"
        );
        std::thread::sleep(Duration::from_millis(250));
    }
    ensure!(
        !boop::live::pid_alive(old_pid as u32),
        "old frontend survived exit"
    );
    let answer = format!(
        "STALE_ACK_{}_{}_{}",
        harness.entry(),
    ...
    let message = bus::messages_in(&fixture.store()?)?
        .into_iter()
        .find(|message| message.body == body)
        .context("stale-route envelope absent")?;
    ensure!(
        !fixture
            .store()?
            .delivery_accepted(&message.id, &fixture.route)?,
        "stale route falsely accepted delivery"
    );
    ensure!(
        !harness
            .observe(registry, &route)?
            .iter()
            .any(|row| row.text.contains(&body)),
        "stale route reached a native transcript before resume"
    );
    fixture.launch(harness, Some(thread))?;
    let resumed = await_route(fixture, harness)?;
    ensure!(
        resumed.session_id == route.session_id,
        "resume selected a different conversation"
    );
    let receipt = await_receipt(fixture, harness, registry, &body, &answer, &sent)?;
    ensure!(
        fixture.store()?.trace_of(thread)?.as_deref() == Some(trace.as_str()),
        "resume changed Boop trace"
    );
    let new_pid = fixture
        .store()?
        .live_row(thread)?
        .and_then(|row| row.pid)
        .context("resumed session has no PID")?;
    ensure!(new_pid != old_pid && boop::live::pid_alive(new_pid as u32));
    Ok(
        json!({"receipt":receipt,"trace":trace,"old_pid":old_pid,"new_pid":new_pid,"graceful_exit":true,"held_while_stale":true}),
    )
}
...
fn authenticated_matrix() -> Result<()> {
    ...
    let harness: Box<dyn LifecycleHarness> = match entry.as_str() {
        "codex" => Box::new(Codex),
        "claude" => Box::new(Claude { entry: "claude" }),
        "ccz" => Box::new(Claude { entry: "ccz" }),
    ...
    let mut fixture = Fixture {
        ...
        parent: "e2e-parent".into(),
        owns_server: true,
        root,
        socket: format!("boop-e2e-{}-{}", entry, std::process::id()),
        pane: String::new(),
        route: String::new(),
        cwd,
        launch_index: std::cell::Cell::new(0),
        cleaned: std::cell::Cell::new(false),
    };
    let mut cases = Vec::new();
    let mut active_scenario = "native_executable";
    let result = (|| -> Result<()> {
        let version = bounded(Command::new(harness.entry()).arg("--version")).map_err(|error| {
            anyhow::anyhow!("BLOCKED: native executable prerequisite: {error:#}")
        })?;
        cases
            .push(json!({"scenario":"native_executable","status":"PASS","version":version.trim()}));
        active_scenario = "fresh_wrapper_identity";
        let registry = Registry::discover();
        let _adapter = harness.adapter(&registry);
        fixture.pane = fixture
            .tmux(&[
                "new-session",
                "-d",
                "-P",
        ...
        let trace = fixture
            .store()?
            .trace_of(thread)?
            .context("fresh trace absent")?;
        let pid = fixture
            .store()?
            .live_row(thread)?
            .and_then(|row| row.pid)
            .context("fresh PID absent")?;
        ...
        cases.push(
            json!({"scenario":"idle_receipt_and_accepted_retry","status":"PASS","receipt":receipt}),
        );
        ...
        while !harness.busy(&fixture, &registry)? {
                ...
                Instant::now() < deadline,
                "native busy state not observed within 25 seconds"
            );
            std::thread::sleep(Duration::from_millis(100));
        }
        ...
        let rows = harness.observe(&registry, &fixture.route()?)?;
            ...
            rows.iter()
                .filter(|row| row.role == "assistant"
                    && row.text.lines().any(|line| line.trim() == busy_answer))
                .count()
                == 1,
        ...
        while harness.compact_count(&registry, &fixture.route()?)? <= before {
                ...
                Instant::now() < deadline,
                "native compaction receipt not observed within 65 seconds"
            );
            std::thread::sleep(Duration::from_millis(500));
        }
        ensure!(
            fixture.route()?.session_id == route.session_id,
            "compact changed native conversation"
        );
        ...
        match harness.change_settings(&fixture, &registry) {
            Ok((model, effort)) => {
                let receipt = nonce(&fixture, harness.as_ref(), &registry, "settings")?;
                    ...
                    receipt["model"] == model && receipt["effort"] == effort,
                    "requested settings differ from native execution"
                );
                cases.push(
                    json!({"scenario":"model_and_effort_change","status":"PASS","receipt":receipt}),
                );
                active_scenario = "settings_across_resume";
                let receipt = resume(&fixture, harness.as_ref(), &registry, "settings_resume")?;
                ensure!(
                    receipt["receipt"]["model"] == model && receipt["receipt"]["effort"] == effort,
                    "resume did not preserve observed settings"
                );
                cases.push(
        ...
        let thread = before
            .session_id
            .as_deref()
            .context("crash test has no thread")?;
        ...
        let old_pid = fixture
            ...
            .live_row(thread)?
            .and_then(|row| row.pid)
            .context("crash test has no PID")? as u32;
        ensure!(
            fixture.owned_pids()?.contains(&old_pid),
            "refusing to signal PID outside this test pane"
        );
        ...
        if !harness.automatic_restart() {
                ...
                != "1"
            {
                    ...
                    Instant::now() < deadline,
                    "wrapper did not exit after frontend crash"
                );
                std::thread::sleep(Duration::from_millis(250));
            }
        ...
        let restarted = await_route(&fixture, harness.as_ref())?;
            ...
            restarted.session_id == before.session_id && restarted.parent == before.parent,
            "crash restart changed identity linkage"
        );
        let new_pid = fixture
            .store()?
            .live_row(thread)?
            .and_then(|row| row.pid)
            .context("restart has no PID")? as u32;
        ensure!(
            new_pid != old_pid && !boop::live::pid_alive(old_pid),
            "crash did not replace frontend PID"
        );
        let receipt = nonce(&fixture, harness.as_ref(), &registry, "crash_restart")?;
        ensure!(
            receipt["model"] == model && receipt["effort"] == effort,
            "crash restart lost settings"
        );
        ...
        if backend_support == NativeBackendSupport::SeparateProcess {
            ...
            if !automatic {
                    ...
                    != "1"
                {
                        ...
                        Instant::now() < deadline,
                        "wrapper did not exit after backend crash"
                    );
                    std::thread::sleep(Duration::from_millis(250));
                }
                fixture.launch(harness.as_ref(), Some(thread))?;
            ...
            loop {
                ...
                if pid.is_some_and(|pid| pid as u32 != new_pid && boop::live::pid_alive(pid as u32))
                {
                    break;
                }
                ensure!(
                    Instant::now() < deadline,
                    "backend restart did not replace the frontend"
                );
                std::thread::sleep(Duration::from_millis(250));
            }
            let restored = await_route(&fixture, harness.as_ref())?;
            ensure!(
                restored.session_id == before.session_id && restored.parent == before.parent,
                "backend restart changed linkage"
            );
            ensure!(
                !boop::live::pid_alive(backend) && !boop::live::pid_alive(new_pid),
                "old backend/frontend survived restart"
            );
            let receipt = nonce(&fixture, harness.as_ref(), &registry, "backend_restart")?;
            ensure!(
                receipt["model"] == model && receipt["effort"] == effort,
                "backend restart lost settings"
            );
        ...
        let before_clear = fixture
            .route()?
            .session_id
            .context("clear recovery thread absent")?;
        let clear_result = (|| -> Result<()> {
            ...
            while fixture
                ...
                .session_id
                .as_ref()
                .is_none_or(|id| Some(id) == old.session_id.as_ref())
            {
                ensure!(
                    Instant::now() < deadline,
                    "clear did not rebind to a new native session within 25 seconds"
                );
                std::thread::sleep(Duration::from_millis(250));
            }
            let new = await_route(&fixture, harness.as_ref())?;
                ...
                fixture
                    .store()?
                    .trace_of(new.session_id.as_deref().unwrap())?
                    == trace,
                "clear changed Boop trace"
            );
        ...
        if let Err(error) = clear_result {
            cases.push(
                json!({"scenario":active_scenario,"status":"FAIL","detail":format!("{error:#}")}),
            );
                ...
                != "1"
            {
                    ...
                    Instant::now() < deadline,
                    "clear failure recovery could not stop its test-owned frontend"
                );
                std::thread::sleep(Duration::from_millis(250));
            }
            fixture.launch(harness.as_ref(), Some(&before_clear))?;
            ensure!(
                await_route(&fixture, harness.as_ref())?
                    .session_id
                    .as_deref()
                    == Some(before_clear.as_str()),
                "clear failure recovery rebound another conversation"
            );
        }
        ...
        let other = Fixture {
            ...
            parent: "e2e-parent".into(),
            owns_server: false,
            route: format!("{}-{}", harness.id(), pane.trim_start_matches('%')),
            pane,
        ...
        let first = fixture.route()?;
            ...
            first.session_id != second.session_id
                && first.tmux != second.tmux
                && first.cwd == second.cwd,
            "concurrent identities collided"
        );
        ensure!(
            fixture
                .store()?
                .trace_of(first.session_id.as_deref().unwrap())?
                != fixture
                    .store()?
                    .trace_of(second.session_id.as_deref().unwrap())?,
            "unrelated concurrent sessions share a trace"
        );
        ...
        let second_rows = harness.observe(&registry, &second)?;
            ...
            !first_rows.iter().any(|row| row.text.contains(&format!(
                "ACK_BOOP_E2E_{}_{}_isolation_b",
                harness.entry(),
                std::process::id()
            ))),
            "second-session message leaked into first transcript"
        );
        ensure!(
            !second_rows.iter().any(|row| row.text.contains(&format!(
                "ACK_BOOP_E2E_{}_{}_isolation_a",
                harness.entry(),
                std::process::id()
            ))),
            "first-session message leaked into second transcript"
        );
        ...
        fixture.parent = other.route.clone();
            ...
            fixture.route()?.parent.as_deref() == Some(other.route.as_str()),
            "child parent edge was not updated"
        );
        ...
        let receipt = await_receipt(
            ...
            &completion,
            &answer,
            "completion command executed by live child",
        )?;
        let results: Vec<_> = bus::messages_in(&fixture.store()?)?
            .into_iter()
            .filter(|message| {
                message.from == fixture.route
                    && message.to == other.route
                    && message.kind == "result"
            })
            .collect();
        ensure!(
            results.len() == 1 && results[0].rc == Some(0),
            "child completion envelope is missing or duplicated"
        );
        ...
        cases.push(
            json!({"scenario":"concurrent_process_cleanup","status":"PASS","receipt":cleanup}),
        );
        Ok(())
    })();
    ...
    match &cleanup {
        Ok(receipt) => {
            cases.push(json!({"scenario":"process_cleanup","status":"PASS","receipt":receipt}))
        }
        Err(error) => cases.push(
            json!({"scenario":"process_cleanup","status":"FAIL","detail":format!("{error:#}")}),
        ),
    }
    for scenario in [
        "fresh_wrapper_identity",
        "idle_receipt_and_accepted_retry",
        "busy_receipt",
        "exit_and_resume_across_processes",
        "compact",
        "resume_after_compact",
        "clear_new_session",
        "resume_after_clear",
        "model_and_effort_change",
        "settings_across_resume",
        "abnormal_exit",
        "stale_route",
        "backend_restart",
        "concurrent_session_isolation",
        "child_completion_parent_receipt",
    ] {
    ...
    std::fs::write(
        fixture.root.join("matrix.json"),
        serde_json::to_vec_pretty(&json!({"entry":entry,"build":boop::BUILD,"cases":cases}))?,
    )?;
    eprintln!(
        "live matrix: {}",
        fixture.root.join("matrix.json").display()
    );
    result?;
    cleanup?;
    let incomplete: Vec<_> = cases
        .iter()
        .filter(|case| matches!(case["status"].as_str(), Some("FAIL" | "BLOCKED")))
        .filter_map(|case| case["scenario"].as_str())
        .collect();
    ensure!(
        incomplete.is_empty(),
        "live coverage incomplete: {incomplete:?}"
    );
    Ok(())
}
