...
/// The one direct generate/check path covering both characters' attribute
/// modules. Each character contributes its own retained `attributes.html`; the
/// reusable [`attribute_source`] derivation embeds no character name.
fn character_attributes(check: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pigeon = attribute_source(include_str!("fighters/pigeon/imported/attributes.html"))?;
    emit_text(&root.join("pigeon/generated/1_attributes.rs"), &pigeon, check)?;
    let dog = attribute_source(include_str!("fighters/dog/imported/attributes.html"))?;
    emit_text(&root.join("dog/generated/2_attributes.rs"), &dog, check)?;
    Ok(())
}
