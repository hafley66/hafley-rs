...
Port progress lives in the checked static dashboard
`classification/6_progress.html`, regenerated by `just progress` and rejected as
stale by `just test`. Retained ingest hashes, declared frame counts and package
stages are derived; only semantic mechanic dispositions are authored, in
`classification/4_progress.tsp`. When an increment changes what works, update
