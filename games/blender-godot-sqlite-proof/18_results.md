...
## Native PM Pigeon fixture

`bash pigeon-lab/3_run.sh` runs the additional isolated Rust package and records
`pigeon-lab/5_pigeon_knee.mp4` without Godot or Bevy. Published brawllib_rs 0.29.0
...
outside this fixture. See `pigeon-lab/7_readme.md` for precise boundaries.

## Input-driven Pigeon and GGRS peers

`bash pigeon-lab/9_run_rollback.sh` adds a separate increment without replacing
...
`pigeon-lab/12_rollback.mp4` displays both peers with input, action, pose, damage,
confirmation, and rollback diagnostics. See `pigeon-lab/14_rollback_readme.md`.

## Sandbag launch and landing

`bash pigeon-lab/15_run_launch.sh` uses ssbm_utils 0.4.0 for knockback, launch
...
`pigeon-lab/18_launch.mp4` adds target phase, timer, position, velocity, and grounded
labels. This is a mixed PM-data/Melee-calculation/Rapier fixture, with approximate
movement rules. See `pigeon-lab/20_launch_readme.md` for parameters and boundaries.

## Recycled SQLite presentation

`bash pigeon-lab/22_run_sql.sh` tests and records the delayed peer with wgpu
...
Eight Pigeon tests and eight core tests pass. Storage bounds apply to the
publication buffers; this fixture still allocates simulation snapshots and
rendering data. `pigeon-lab/23_sql_boundary.mp4` labels publication generations,
reader state, action/pose, damage, and rollback. See `pigeon-lab/26_sql_readme.md`
for schema, replay assertions, cursor-isolation scope, and reproduction commands.

## Godot consumption through gdext

`bash pigeon-lab/29_run_godot.sh` builds `godot` 0.4.5 against the 4.5 API and
...
`pigeon-lab/31_godot_sql.mp4` records the Godot presentation; the 180 simulation
...
`pigeon-lab/33_godot_readme.md` for thread ownership, capture details, and scope.
...
ticks 105 and 128 reproduces every subsequent state. `pigeon-lab/41_incremental.mp4`
and `pigeon-lab/43_incremental_readme.md` contain the capture and boundaries.

## Independent worker and stalled consumption

`bash pigeon-lab/46_run_schedule.sh` drives fixed simulation steps from a Rust
...
Thirteen Pigeon tests pass, and all-target gdext Clippy passes with warnings denied.

`pigeon-lab/49_schedule.mp4` displays simulation/display lag, publication refusal,
...
`pigeon-lab/52_schedule_readme.md` for ownership, policy, and reproduction.

## Main-thread, render-thread, and process suspension

`bash pigeon-lab/55_run_faults.sh` adds bounded 800 ms delays on the Godot main
...
`pigeon-lab/60_faults.mp4` records the resulting frame jumps and recovery;
`pigeon-lab/58_faults.json` carries wall-clock evidence that MovieMaker's
...
remain untested. See `pigeon-lab/63_fault_readme.md` for reproduction and scope.
