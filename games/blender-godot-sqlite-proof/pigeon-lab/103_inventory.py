...
SOURCE_CHARACTER = "/PM3.6/Captain Falcon/"
...
def inventory(root):
    ...
    for file in sorted(root.rglob("*")):
        ...
        for ref in refs:
            ...
            if path is None:
                ...
                continue
            links.add(path)
            if relative == "PM3.6/Captain Falcon/subactions.html" and path.startswith(SOURCE_CHARACTER + "subactions/"):
                pigeon_expected.add(path)
    ...
    missing_pigeon = [path for path in sorted(pigeon_expected)
    ...
    return dict(version=1, network_requests=0, files=list(rows.values()),
                summary=dict(files=len(rows), bytes=sum(r["bytes"] for r in rows.values()),
                             ...
                             pigeon_subactions_expected=len(pigeon_expected),
                             pigeon_subactions_missing=len(missing_pigeon)),
                pigeon_index_present="PM3.6/Captain Falcon/subactions.html" in rows,
                pigeon_missing=missing_pigeon, missing=missing, external=sorted(external),
