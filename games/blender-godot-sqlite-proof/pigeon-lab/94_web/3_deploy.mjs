...
const head = p => `<base href="${p.BASE_PATH}">\n<script>window.PIGEON_DEMO = new URL(location.href).searchParams.has("demo");</script>\n`;
export const app = createWebApp({ root, describe, head, profile, packageName: 'pigeon_web',
  requiredSprites: [], webAssets: [], reloadCommand: 'nginx -t' });

export function stage() {
  mkdirSync(resolve(root, 'godot/bin'), { recursive: true });
  for (const [from, to] of [
    ['2_project.godot', 'project.godot'], ['2_extension.gdextension', '0_pigeon.gdextension'],
