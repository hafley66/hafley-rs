...
pub fn run(
    ...
    for (tick, expected) in golden.iter().enumerate() {
        clock(tick);
        let pair = runtime.advance(pigeon::fixture_input(tick as i32))?;
...
pub fn spawn(shared: State, faults: bool) -> std::thread::JoinHandle<Result<Vec<Status>, String>> {
    ...
    std::thread::spawn(move || {
        ...
        let worker =
            tracing::info_span!(target: "pigeon::worker", parent: &parent, "worker", faults);
        ...
        run(!faults, &shared, |tick| {
            ...
            tracing::debug!(target: "pigeon::worker", parent: &worker, tick, elapsed_us = start.elapsed().as_micros() as u64, "tick_due");
            if faults {
