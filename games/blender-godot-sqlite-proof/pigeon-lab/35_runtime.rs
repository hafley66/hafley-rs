...
fn input(tick: i32) -> u8 {
    pigeon::fixture_input(tick)
}
use pigeon::advance_world as step;
...
#[tracing::instrument(target = "pigeon::rollback", level = "trace", skip_all, fields(tick = world.frame))]
    ...
    for request in requests {
        match request {
            GgrsRequest::SaveGameState { cell, frame } => {
                let _span = tracing::trace_span!(target: "pigeon::rollback", "save", tick = frame)
                    .entered();
            ...
            GgrsRequest::LoadGameState { cell, frame } => {
                let _span =
                    tracing::debug_span!(target: "pigeon::rollback", "restore", tick = frame)
                        .entered();
            ...
            GgrsRequest::AdvanceFrame { inputs } => {
                ...
                rollback::apply_request::<C>(world, GgrsRequest::AdvanceFrame { inputs }, checksum, |state, inputs| {
                    pigeon::advance_world(state, inputs[0].0, baked);
                });
                record(world);
...
impl<'a> Runtime<'a> {
    ...
    #[tracing::instrument(target = "pigeon::runtime", level = "debug", skip_all, fields(tick = self.tick, input = bits))]
    pub fn advance(&mut self, bits: u8) -> Result<[Display; 2], Error> {
        ...
        for id in 0..2 {
            let _span =
                tracing::debug_span!(target: "pigeon::rollback", "peer", peer = id).entered();
            ...
            tracing::debug!(target: "pigeon::rollback", advances = peer_display.advances, restores = peer_display.restored.len(), saves = peer_display.saved.len(), confirmed = peer_display.confirmed, "requests_executed");
...
pub(crate) fn render(actions: &[HighLevelSubaction], trace: &[[Display; 2]], id: usize) -> Result<(), Error> {
    ...
    for (tick, pair) in trace.iter().enumerate() {
        ...
        text(
            &mut vertices,
            if launch {
                "0.5X + HOLDS / SCRIPTED PIGEON / PM DATA + MELEE KB + RAPIER"
            } else {
