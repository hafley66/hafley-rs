...
impl Boundary {
    ...
    #[tracing::instrument(target = "pigeon::sql", level = "trace", skip_all, fields(generation = self.generation + 1, frames = frames.len()))]
    ...
    fn publish_rows(&mut self, rows: impl Iterator<Item = Row> + Clone) -> contracts::PublishResult {
        ...
        if total > ROW_CAPACITY {
            tracing::warn!(target: "pigeon::sql", rows = total, capacity = ROW_CAPACITY, reason = "row_capacity", "publication_refused");
        ...
        if self
            ...
            .is_none()
        {
            tracing::warn!(target: "pigeon::sql", rows = total, reason = "slots_pinned", "publication_refused");
            return Err(BoundaryError::SlotsPinned);
        }
        std::mem::swap(&mut self.history, &mut self.scratch);
        self.generation = next;
        tracing::trace!(target: "pigeon::sql", generation = next, rows = total, "published");
...
impl contracts::RowPublisher for Boundary {
    #[tracing::instrument(target = "pigeon::sql", level = "trace", skip_all, fields(rows = rows.len()))]
...
#[tracing::instrument(target = "pigeon::sql", level = "trace", skip_all)]
...
#[tracing::instrument(target = "pigeon::sql", level = "trace", skip_all, fields(tick))]
