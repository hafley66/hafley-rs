...
BINARY = LAB / "target/debug/pigeon-rollback"
...
def main(lifecycle):
    ...
    def spawn_renderer():
        ...
        child = subprocess.Popen([
            ...
            "--", "--external"], stdout=log, stderr=log, env={**os.environ,
                "PIGEON_LIVE_PATH": str(root / "live-1.bin"),
                "PIGEON_LIVE_AUDIT": str(root / f"acks-{index}.jsonl"),
                "PIGEON_LIVE_NOTE": str(root / "note.txt"),
                "PIGEON_LIVE_STOP": str(root / f"stop-{index}"), "RUST_LOG": "warn"})
    ...
    try:
        ...
        relay = subprocess.Popen([sys.executable, str(LAB / "75_udp_lab.py"), str(BINARY)],
            stdout=relay_log, stderr=relay_log, env={**os.environ, "PIGEON_LIVE_DIR": str(root)})
        while True:
