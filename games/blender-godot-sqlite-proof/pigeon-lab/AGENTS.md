...
worlds and Smash-style combat, verified through playable Pigeon increments.
...
Each slice wires its extracted machinery into Pigeon immediately. Keep other
...
- Before implementing a feature, identify its shared mechanism and game-owned
  ...
  must not import Pigeon modules or embed its paths, action IDs or expectations.
- Pigeon consumes the shared implementation in the same change. Game schemas,
...
- Cover shared behavior independently and exercise the real Pigeon consumer.
  Generator tooling also needs a small non-Pigeon schema fixture. Keep ownership,
