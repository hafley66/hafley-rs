...
## Immediate slice: buffered input to Pigeon action
...
No rxRust work is requested. Pigeon runtime remains unchanged.
...
delayed remote-input correction, network transport, Pigeon integration and MP4
...
inspection found Pigeon `simulation-core/2_simulation.rs::World` derives Serde
...
negative Serde regressions. Pigeon integration must explicitly preserve restore
...
| I2 | Done | Extract required input machinery into `games/shared` and wire Pigeon | Shared input buffer consumed by existing Redux Step; generated policy/inspection types; full clone/JSON/bincode restore and GGRS/SQL tests pass. See increment below. |
...
Pigeon mapping is proposed in I1. Do not assume the existing input crate already
...
| G1 | Deferred | Existing Pigeon subset verified; full-site completion, retrieval metadata and real downloader recovery remain unverified. Resume for a concrete missing/corrupt asset or explicit acquisition request. Deferral does not establish background downloader status. |
| G2 | Pending | Expand versioned Pigeon package and supported behavior after input slice; 491 decoded subactions, seven runtime actions. Full mirror is not a prerequisite. |
...
| P1 | Pending | Shared TSP generation glue, non-Pigeon fixture and Pigeon consumer. |
...
inputs; uninitialized submodules have an explicit marker. The Pigeon workflow
...
| --- | --- | --- |
| Quantized participant input | `crates/input/src/{0_types,1_quantize}.rs`: four i8 axes, u32 buttons and conversion | Ported to shared/input; richer four-axis Pigeon mapping remains pending |
...
uses macro-free statig with no entry/exit actions. Pigeon World owns the optional
...
`just test core` passed: `.workflow/test-AXxLQg/receipt.json`, 28 Pigeon tests,
