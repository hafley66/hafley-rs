//! Same Pigeon input tape, selected buffering policy, real SQL presentation and GGRS replay.
use smash::fighters::pigeon;
use crate::fixture::{self, baseline, sql_viewer};
use pigeon::{Action, InputBuffer, World};
...
impl RollbackSim for Sim {
    ...
    fn advance(state: &World, inputs: &[u8], cx: &Config) -> World {
        ...
        pigeon::advance_world(&mut next, inputs[0] & 3, &cx.actions);
        next
    }
