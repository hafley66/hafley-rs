...
#[tracing::instrument(target="pigeon::ipc", level="trace", skip_all, fields(generation=latest.generation, rows=latest.rows.len()))]
...
#[tracing::instrument(target = "pigeon::ipc", level = "trace", skip_all)]
...
mod tests {
    ...
    fn latest_replaces_without_backlog_and_preserves_float_bits() {
        use super::*;
        let dir = std::env::temp_dir().join(format!("pigeon-live-test-{}", std::process::id()));
