...
use pigeon::{World, fixture_input};
use ggrs::{
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut loads = 0;
    let live_path = std::env::var_os("PIGEON_LIVE_DIR")
    ...
    for tick in 0..200 {
        ...
        let _span = tracing::debug_span!(target: "pigeon::network", "process_tick", peer=id, tick)
            .entered();
