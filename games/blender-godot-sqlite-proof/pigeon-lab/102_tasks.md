...
| --- | --- | --- | --- | --- |
| G0 | Done | Reuse redux and GGRS execution | Existing libraries | `62832dc`; `.workflow/test-uAKBba/receipt.json`: redux unit/external tests, shared rollback replay, 27 Pigeon tests |
| G1 | In progress | Verify local mirror and Pigeon inventory | G0 | Offline inventory and Pigeon decoding passed; full mirror and downloader truncation recovery remain open |
| G2 | Pending | Full available Pigeon data and behavior coverage | Pigeon subset of G1 | Versioned package, decoder tests, supported/unresolved callback report |
...
its verified Pigeon subset. No existing rollback proof is discarded.

## Prefactoring gates, added 2026-09-08

These gates apply before extending the corresponding mechanism for G1-G7. Shared
implementation and Pigeon consumption land together, without waiting for another
game. G1-G7 remain open with their existing IDs and evidence.

| ID | State | Shared mechanism | Acceptance |
| --- | --- | --- | --- |
| P1 | Pending | TSP row/Godot generation glue | Reuse existing emitter; game supplies schema; non-Pigeon schema fixture plus unchanged Pigeon outputs and consumer tests |
| P2 | Pending | SQLite ring/query/publication | Reuse core-labs storage; game supplies layout/encoding; pinning, exhaustion, replay replacement and query-error tests pass through Pigeon |
| P3 | Pending | Capture/MP4 and workflow receipts | Shared runner accepts game commands/scenario/artifacts; existing encoder/probe reused; Pigeon recording and failure/staleness checks pass without shared Pigeon paths |
...
- All 491 Pigeon subaction index links resolve locally. `just import-catalog`
...
- Catalog metadata: `.workflow/pigeon-catalog.json`; diagnostics:
  `.workflow/pigeon-catalog.log`. This is a data/compatibility audit; the playable
...
- `just test core` passed, receipt `.workflow/test-o8sEcu/receipt.json`, including
  shared-library and 27 Pigeon tests. Offline inventory tests cover missing,
...
- [ ] Reuse brawllib for all available Pigeon subactions, attributes, actions,
...
- [ ] Inventory common movement and Pigeon callback dependencies. Reuse existing
...
One input tape: Pigeon acquires boots, gains an air jump, uses it, drops the boots,
