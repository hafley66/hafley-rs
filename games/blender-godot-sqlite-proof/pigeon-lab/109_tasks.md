...
| --- | --- | --- |
| L1 | Done | Shared replay/restore harness consumed by Pigeon; just test-reuse and scoped buffer_proof test pass. |
| N1 | Done | Held attack consumes at 25 only; release 64/repress 65 consumes at 25/65. Both scoped Pigeon tests pass with GGRS/SQL and six clone/JSON/binary restore checkpoints. |
...
L2 will retain game label content and pixel assertions in Pigeon. Shared code
...
Disk exhaustion interrupted the ledger write; scoped cargo clean -p pigeon-lab
...
tests cover panel coordinates and text translation; Pigeon assertions remain local.
API/lifetime details are in `../../shared/0_reuse.md`.

Executed `just test core`: 29 Pigeon tests, 2 capture tests, 2 rollback tests,
