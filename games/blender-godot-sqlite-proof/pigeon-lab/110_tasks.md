...
| F3 | Done, bounded | `3dc9886`: shared fighter Redux state consumed by Pigeon; `just test core` passed, `.workflow/test-UrhSp3/receipt.json`. Source-equivalence limits remain in domain ledger. |
...
F1/F2 qualify the selected locomotion data only. Full Pigeon callback execution
...
PigeonSql controlled ticks and generated Rows/Payload adapters. Independent
...
Package: Pigeon lab (app proof), stage 2.7 -> 2.7. Property gained: actual
...
two opposing digital sides by last-pressed order, ignoring key-repeat, with a
focus reset that suspends output until a fresh press; `2_stage.gd` feeds it key
events and touch buttons. Native
