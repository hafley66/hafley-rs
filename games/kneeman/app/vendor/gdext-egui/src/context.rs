...
struct SharedContext {
    ...
    /// EGUI_PASS_LOG=1 counters. See `PassLogCounters`. Zero-cost when unset: every call
    /// site checks `pass_log_enabled()` (one `OnceLock` read) before touching this.
    pass_log: PassLogCounters,
}

/* --------------------------------- EGUI_PASS_LOG diagnostic -------------------------------- */
//
// plans/egui-repaint-lab.md Part 2: per-second summary of pass volume in the real game, to
// check A1 (does gdext-egui run a full pass every process() frame regardless of input) against
// something other than the headless lab. Inert unless `EGUI_PASS_LOG` is set; additive only,
// no change to the default code path.
//
// Scope note: "tessellate_ms" times only the `Context::tessellate` call below, not the whole
// pass. The UI-building closure (`egui::Window::show` etc.) runs in caller code -- shell/menu,
// outside this crate -- so gdext-egui has no hook to time it from here.

/// Reads `EGUI_PASS_LOG` once and caches the result; every instrumentation call site gates on
/// this first so the disabled path costs one atomic-free `OnceLock::get_or_init` load.
fn pass_log_enabled() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| std::env::var_os("EGUI_PASS_LOG").is_some())
}

/// Root-viewport-only counters (sub-viewport windows aren't counted) drained and printed once
/// per second of wall time.
#[derive(Default)]
struct PassLogCounters {
    passes: AtomicU64,
    input_events: AtomicU64,
    shapes: AtomicU64,
    tessellate_ns: AtomicU64,
    window_start: Mutex<Option<Instant>>,
}

impl PassLogCounters {
    /// Prints and resets the counters if >=1s of wall time elapsed since the last report.
    fn maybe_report(&self) {
        ...
        let passes = self.passes.swap(0, Relaxed);
        let input_events = self.input_events.swap(0, Relaxed);
        let shapes = self.shapes.swap(0, Relaxed);
        let tessellate_ms = self.tessellate_ns.swap(0, Relaxed) as f64 / 1e6;

        godot_print!(
            "[EGUI_PASS_LOG] passes/s={passes} input_events={input_events} \
             shapes={shapes} tessellate_ms={tessellate_ms:.3}"
        );
...
impl EguiBridge {
    ...
    fn viewport_start_frame(&self, id: ViewportId) {
...

        {
            ...
            let n_events_before = raw_input.events.len();
            raw_input.events.extend(viewport.rx_update.try_iter());
            n_events = raw_input.events.len() - n_events_before;
        ...
        if pass_log_enabled() && id == ViewportId::ROOT {
            self.share.pass_log.passes.fetch_add(1, Relaxed);
            self.share
                .pass_log
                .input_events
                .fetch_add(n_events as u64, Relaxed);
        }
    ...
    fn viewport_end_frame(&self, id: ViewportId) {
        ...
        let primitives = if pass_log_enabled() && id == ViewportId::ROOT {
            let n_shapes = paints.len() as u64;
            let t0 = Instant::now();
            let primitives = self.share.egui.tessellate(paints, ppi);

            self.share.pass_log.shapes.fetch_add(n_shapes, Relaxed);
            self.share
                .pass_log
                .tessellate_ns
                .fetch_add(t0.elapsed().as_nanos() as u64, Relaxed);
            self.share.pass_log.maybe_report();

            primitives
        } else {
            self.share.egui.tessellate(paints, ppi)
        };
        self.share
            .viewports
            .lock()
