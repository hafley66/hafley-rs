...
use crate::sim::world::fold::chain;
use crate::sim::world::store::{SCHEMA, Slot, WorldStore};
use crate::sim::world::{
    ASSET_MAX, AssetErr, AssetId, EventId, Node, PlayerId, Seed, WorldEvent, WorldId, asset_id,
    canon, decanon, event_id, world_id,
};
...
pub fn load_or_make_owner() -> PlayerId {
    ...
    if cfg.has_section_key("player", "key")
        && let Ok(h) = cfg.get_value("player", "key").try_to::<GString>() {
    ...
    cfg.set_value(
        "player",
        "key",
        &GString::from(hex32(&id).as_str()).to_variant(),
    );
...
impl WorldStore for GodotStore {
    ...
    fn append(&mut self, id: WorldId, ev: &WorldEvent) -> EventId {
        let Some(c) = self.worlds.get_mut(&id) else {
            return event_id(None, SCHEMA, &canon(ev));
        };
    ...
    fn since(&self, id: WorldId, from: Option<EventId>) -> Vec<Node> {
        let Some(c) = self.worlds.get(&id) else {
            return Vec::new();
        };
        match from {
            None => c.nodes.clone(),
    ...
    fn has(&self, id: WorldId, ev: EventId) -> bool {
        self.worlds
            .get(&id)
            .is_some_and(|c| c.nodes.iter().any(|n| n.id == ev))
    }
    ...
    fn cache_bytes(&self, id: WorldId) -> u64 {
        let file_len = |path: String| -> u64 {
            FileAccess::open(&GString::from(path.as_str()), ModeFlags::READ)
                .map_or(0, |f| f.get_length() as u64)
        };
