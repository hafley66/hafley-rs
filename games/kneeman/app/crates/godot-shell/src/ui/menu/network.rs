...
impl Screen for Network {
    fn view<T: Theme>(&self, ui: &mut egui::Ui, theme: &T, cx: &MenuCtx, out: &mut Vec<Intent>) {
        ...
        let chip_label = if offline {
            "OFFLINE".to_string()
        } else {
            format!("ONLINE  ·  {}", net.phase)
        };

        egui_rsx! {
            stylesheet: "xp.css",
            div { class: "groupbox",
                label { class: "legend", "Network" }
                {
                    // Chip frame + multi-line status + action buttons + lobby grid. Heavy on RichText
                    // styling + Grid; raw-expr escape hatch keeps the body verbatim under the rsx chrome.
                    ui.horizontal(|ui| {
                        egui::Frame::NONE
                    ...
                    ui.horizontal(|ui| {
                        if offline {
                            ...
                            if theme.button(ui, "Copy invite link").clicked() {
                                out.push(Intent::CopyInviteLink);
                            }
                    ...
                    egui::Grid::new("lobby_grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            for row in lobbies {
                                ...
                                ui.label(
                                    RichText::new(format!("{}/{}", row.active, row.cap)).color(if full {
                                ...
                                match row.ttl_remaining_secs(now) {
                                    Some(s) => {
                                        ui.label(RichText::new(format!("culls {s}s")).color(ttl_color(s)))
                                    }
                                    None => {
                                        ui.label(RichText::new("live").color(Color32::from_rgb(120, 200, 130)))
                                    }
                                };
                                if row.host == "you" {
                                    ui.add_enabled(false, egui::Button::new("Joined").small());
                                } else if ui
                                    .add_enabled(!full, egui::Button::new("Join").small())
                                    .clicked()
                                {
