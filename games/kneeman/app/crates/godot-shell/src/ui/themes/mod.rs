...
pub trait Theme {
    ...
    /// A titled top-level container, centered on screen and clamped to fit the viewport. `add` fills
    /// the body below the title bar; its `egui::Vec2` argument is the room left for that body after
    /// the clamp and the title bar are accounted for, so the caller can decide compact vs. roomy
    /// layout and bound its own scroll areas without re-deriving the chrome's fixed costs.
    fn window(&self, ctx: &egui::Context, title: &str, add: impl FnOnce(&mut egui::Ui, egui::Vec2));
