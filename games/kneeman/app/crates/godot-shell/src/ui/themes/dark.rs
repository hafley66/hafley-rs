...
impl super::Theme for Dark {
    ...
    fn window(
        ...
        // egui::Window already clamps its max size against the viewport on every layout pass (not
        // just a first invisible frame like Area's `constrain`), so it needs no extra clamp here.
        // This impl is unused by the pause menu (which always draws through `Xp`) but is kept
        // trait-complete; `ui.available_size()` stands in for `Xp`'s body-size hint. The debug
        // panel's own ad hoc `egui::Window`s (our_stuff_window, saves_window, ...) get the same
        // stable-physical-size fix `Xp::frame_window` needed for free: both themes share one
        // `egui::Context`, and `ui/debug.rs::process()` counter-scales it (see ui/scale.rs)
        // before ANY window is drawn -- no per-theme change needed here.
