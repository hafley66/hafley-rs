...
pub struct DebugUi {
    ...
    menu_nav: Option<Route>, // a shell-side jump request (HUD chip tap -> CharEdit); resolved like menu_esc
    ...
    ui_scale: Mutable<f32>, // client-local UI scale (bigger = bigger UI), persisted to user://; the
                        // manual override on top of the auto counter-scale so tiny/huge UI is always a slider-drag away
}
...
impl INode for DebugUi {
    fn init(base: Base<Node>) -> Self {
        Self {
            ...
            ui_scale: Mutable::new(crate::identity::load_ui_scale()),
        }
    }
    ...
    fn ready(&mut self) {
        ...
        http.connect(
            "request_completed",
            &Callable::from_object_method(&cb, "on_status"),
        );
    ...
    fn process(&mut self, dt: f64) {
        ...
        // (this bridge's CanvasLayer included) by the window/design ratio a second time after
        // egui rasterizes; shrinking `pixels_per_point` by that same ratio cancels it out. Real
        // window size comes from the Window node (unaffected by the CanvasItem stretch);
        // `ctx.content_rect()` stays design-space-derived until this call lands.
        if let Some(window) = self.base().get_window() {
            let real = window.get_size();
            let stretch = crate::ui::scale::stretch_factor(
                (real.x as f32, real.y as f32),
                (
                    crate::ui::scale::DESIGN_WIDTH,
                    crate::ui::scale::DESIGN_HEIGHT,
                ),
            );
            // `get_size` is PHYSICAL px, so `stretch` silently includes the display's hiDPI
            // scale; hand it back so a point lands at one LOGICAL px (see scale.rs's doc --
            // the retina "menu so tiny" bug). On web `screen_get_scale` returns 1.0 (gdext never
            // wires in the browser dpr), so read `window.devicePixelRatio` directly there or the
            // whole UI ships at half size; native uses the engine backing scale. See
            // `net::device_pixel_ratio`.
            let dpr = crate::net::device_pixel_ratio();
            // Manual override: `ui_scale` is user-facing "bigger = bigger UI", but higher ppp renders
            // SMALLER here (canvas_items double-scale, see scale.rs), so divide. A slider away from
            // correct on any platform, regardless of the auto counter-scale.
            let ppp =
                crate::ui::scale::counter_scale_ppp(stretch, dpr) / self.ui_scale.get().max(0.1);
        ...
        // --- XP menu: drawn every frame (independent of the debug panel toggle) ---
        {
            ...
            let esc_resolved = std::mem::take(&mut self.menu_esc);
            if esc_resolved {
                self.pad_held = None;
            ...
            // resolve a shell-side jump (HUD chip tap -> CharEdit) the same deferred way.
            if let Some(r) = self.menu_nav.take() {
                self.router.apply(vec![Intent::Nav(r)], &cells);
            }
            ...
            // kbd_esc: let the menu's own Escape-consume dismiss it, EXCEPT on the frame a shell
            // menu_esc just resolved (the press that opened the menu is still in egui's queue).
            crate::ui::menu::menu(
                &ctx,
                &Xp,
                &mut self.router,
        ...
        fighter.bind_mut().draw_debugger(&ctx);
...
impl DebugUi {
    ...
    /// menu on its last page; from inside the menu it dismisses. Deferred to process() like
    /// menu_esc, so egui is not poked mid-input.
    pub fn open_pause_menu(&mut self) {
        self.menu_esc = true;
    }

    /// Jump the menu straight to a fighter's edit page (the HUD chip tap). Deferred to process()
    /// like menu_esc, so egui is not poked mid-input.
    pub fn open_char_edit(&mut self, slot: u8) {
        self.menu_nav = Some(Route::CharEdit { slot });
    }

    /// HUD damage-chip `gui_input` handler (KneeMan connects each chip with its slot bound): the
    /// chip is the fighter's edit control, so a click/tap opens the menu on that slot's CharEdit
    /// page. `gui_input` only fires inside the chip's rect. Touch emulation may deliver both a
    /// touch and a mouse press for one tap; both land on the same route, so the double fire is
    /// harmless.
    #[func]
    fn on_hud_input(&mut self, event: Gd<InputEvent>, slot: i64) {
        let pressed = match event.clone().try_cast::<InputEventMouseButton>() {
            Ok(m) => m.is_pressed() && m.get_button_index() == MouseButton::LEFT,
            Err(e) => e
                .try_cast::<InputEventScreenTouch>()
                .map(|t| t.is_pressed())
                .unwrap_or(false),
        };
        if pressed {
            self.open_char_edit(slot.clamp(0, 255) as u8);
        }
    }
    ...
    fn on_status(
        ...
        if code != 200 {
            self.server_status
                .set(format!("request failed (HTTP {code})"));
            return;
        }
        self.server_status
            .set(body.get_string_from_utf8().to_string());
    }
}
...
fn draw_saves_window(
    ctx: &egui::Context,
    slots: &[crate::sim::world::store::Slot],
    cache_bytes: u64,
    label: &mut String,
    ...
    egui::Window::new("KneeMan  ·  our stuff")
      ...
      egui::ScrollArea::vertical().max_height(420.0).show(ui, |ui| {
        ...
        egui::CollapsingHeader::new("view scale").default_open(false).show(ui, |ui| {
            dark::card(ui, |ui| {
                let mut ui_scale = ui_scale_cell.get();
                if ui
                    .add(egui::Slider::new(&mut ui_scale, 0.4..=4.0).text("UI scale"))
                    .changed()
                {
                    ui_scale_cell.set(ui_scale);
                    crate::identity::save_ui_scale(ui_scale);
                }
        ...
        Tab::Feel => {
        let mut c = false;
        c |= slider(ui, &mut t.tetris.cell_durability, 0.0..=50.0, "Tetris cell HP (0 = whole outline)");
        ...
        egui::CollapsingHeader::new("dodge / ledge").default_open(false).show(ui, |ui| {
            ...
            c |= slider(ui, &mut t.save_zero_pct, 0.0..=300.0, "save_zero_pct (full reset under)");
            c |= slider(ui, &mut t.save_scale, 0.0..=0.02, "save_scale (vel kept per % over)");
        });
...
fn pretty_status(body: &str) -> Vec<(String, String)> {
    ...
    if !clients.is_empty() {
        ...
        for (idx, chunk) in clients.split('{').skip(1).enumerate() {
            ...
            rows.push((
                format!("  [{idx}]"),
                format!("{name} {color}").trim().to_string(),
            ));
        }
    }
    rows
...
fn gamepad_card(ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| {
        stick_box(
            ui,
            "L stick (move/DI)",
            input.get_joy_axis(dev, JoyAxis::LEFT_X),
            input.get_joy_axis(dev, JoyAxis::LEFT_Y),
        );
        stick_box(
            ui,
            "R stick",
            input.get_joy_axis(dev, JoyAxis::RIGHT_X),
            input.get_joy_axis(dev, JoyAxis::RIGHT_Y),
        );
    });
    ui.add_space(6.0);
    ...
    dark::card(ui, |ui| {
        let lt = input
            .get_joy_axis(dev, JoyAxis::TRIGGER_LEFT)
            .clamp(0.0, 1.0);
        let rt = input
            .get_joy_axis(dev, JoyAxis::TRIGGER_RIGHT)
            .clamp(0.0, 1.0);
        ...
        for (b, label) in pips {
            ...
            ui.horizontal(|ui| {
                ...
                painter.circle_filled(
                    resp.rect.center(),
                    5.0,
                    if on { dark::ACCENT } else { dark::LINE },
                );
...
fn stick_box(ui: &mut egui::Ui, label: &str, x: f32, y: f32) {
    ui.vertical(|ui| {
        ...
        painter.line_segment(
            [egui::pos2(ctr.x - r, ctr.y), egui::pos2(ctr.x + r, ctr.y)],
            grid,
        );
        painter.line_segment(
            [egui::pos2(ctr.x, ctr.y - r), egui::pos2(ctr.x, ctr.y + r)],
            grid,
        );
        let p = egui::pos2(
            ctr.x + x.clamp(-1.0, 1.0) * r,
            ctr.y + y.clamp(-1.0, 1.0) * r,
        );
