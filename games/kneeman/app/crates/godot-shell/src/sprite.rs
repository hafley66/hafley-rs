...
pub(crate) fn make_tag(name: &str, color: Color, font_px: i32) -> Gd<Label> {
    ...
    l.add_theme_color_override(
        "font_outline_color",
        Color::from_rgba(0.05, 0.06, 0.10, 0.92),
    );
...
pub(crate) fn make_edge_tag(color: Color) -> Gd<Label> {
    ...
    l.add_theme_color_override(
        "font_outline_color",
        Color::from_rgba(0.04, 0.05, 0.09, 0.95),
    );
...
pub(crate) fn clip_for(f: &Fighter) -> &'static str {
    ...
    match f.state {
        ...
        Turn => "dashturn",
        Skid => "skid",
        Crouch | JumpSquat | SpotDodge | Shield => "crouch",
        Landing => "landinglag",
        Air | AirDodge => {
            if f.vel.y < 0.0 {
                "jump"
        ...
        Uair => "uair",
        Dair => "dair",
        Fsmash => "fstrong", // Rivals rips name the smashes "strongs"
        Usmash => "ustrong",
        Dsmash => "dstrong",
        DashAttack => "dattack",
        ...
        Getup => "crouch",          // rising
        TechInPlace => "tech",      // braced recovery
        TechWall => "tech",         // wall tech: braced/stuck against the wall (intangible)
        TechRoll => "roll_forward", // rolling across the ground
        ...
        Crawl => "crouch", // creeping: low pose (no crawl art in the pose set)
        LedgeAttack => "dattack", // rising lip swing: the lunge pose reads closest
        LedgeRoll => "roll_forward",
        GetupAttack => "dtilt", // low rising sweep
        Rebound => "skid",      // clanked: the stagger pose
        ShieldBreak => "tech",  // dizzy: braced/stunned pose
    }
}
...
pub(crate) fn clip_fallback(name: &str) -> &'static str {
    match name {
        ...
        "skid" => "idle",
        "nair" => "jab",
        "fair" | "bair" | "uair" => "nair", // any missing aerial reads as the neutral kick
        "jab" => "idle",
        "dair" => "fall",
        "ftilt" | "utilt" => "jab",
        "fstrong" => "ftilt", // smash degrades to its tilt, then the tilt to jab
        "ustrong" => "utilt",
        "dstrong" => "dtilt",
        "dattack" => "run",
        "dashstart" => "run",
        ...
        "tech" => "skid",
        "pratfall" => "fall",
        "nspecial" | "fspecial" | "dspecial" => "jab",
        "uspecial" => "jump",
        "wallbounce" => "fall",
        "fall" => "jump",
        "jump" => "idle",
    ...
    for clip in &c.clips {
        ...
        match &c.sheet {
            Sheet::Strip { frame_px } => {
                if let Some(sheet) =
                    try_tex(&format!("res://assets/{}/{}.png", c.dir, clip.files[0]))
                {
                    ...
                    let fw = if *frame_px > 0.0 {
                        *frame_px
                    } else {
                        sheet.get_width() as f32 / frames as f32
                    };
            ...
            Sheet::Poses { prefix } => {
                for f in &clip.files {
                    if let Some(tex) =
                        try_tex(&format!("res://assets/{}/{}_{}.png", c.dir, prefix, f))
                    {
...
pub(crate) fn validate_character(c: &Character, sf: &Gd<SpriteFrames>) {
    if !sf.has_animation(&StringName::from("idle")) {
        godot_error!(
            "anim: character '{}' has no 'idle' clip — fallbacks have no terminal",
            c.dir
        );
    }
