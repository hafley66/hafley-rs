...
pub(crate) fn falcon() -> Character {
    Character {
        ...
        scale: 1.55,
        offset_y: -45.0, // idle art is 90px tall, centered pivot -> lift half so feet sit on pos
        sheet: Sheet::Strip { frame_px: 0.0 },
        ...
        clips: vec![
            ...
            clip("bair", &["bair_strip2"], 2, 12.0, false),
            clip("uair", &["uair_strip6"], 6, 21.0, false),
            clip("dair", &["dair_strip4"], 4, 12.0, false),
            clip("nspecial", &["nspecial_strip6"], 6, 8.0, false),
            clip("fspecial", &["fspecial_strip11"], 11, 18.0, false),
            clip("uspecial", &["uspecial_strip17"], 17, 18.0, false),
            clip("dspecial", &["dspecial_strip4"], 4, 7.0, false),
        ],
    }
}
...
pub(crate) fn lucas() -> Character {
    Character {
        ...
        offset_y: -6.0,
        sheet: Sheet::Strip { frame_px: 0.0 },
        // Dedicated attack art is a follow-up; missing attack clips use the presentation fallback.
        clips: vec![
            clip("idle", &["idle_strip30"], 30, 14.0, true),
            clip("walk", &["run_strip9"], 9, 14.0, true),
            clip("run", &["run_strip9"], 9, 20.0, true),
            clip("jump", &["jump_strip13"], 13, 12.0, false),
            clip("fall", &["fall_strip2"], 2, 8.0, true),
        ],
    }
}
...
pub(crate) fn load_roster_json() -> Vec<Character> {
    ...
    let Ok(root) = parsed.try_to::<VarDictionary>() else {
        return Vec::new();
    };
    let Some(list) = root
        .get("characters")
...
pub(crate) fn parse_character(d: VarDictionary) -> Option<Character> {
    let dir = jstr(&d, "dir")?;
    ...
    let sheet = match jstr(&d, "sheet").as_deref() {
        Some("poses") => Sheet::Poses {
            prefix: jstr(&d, "prefix").unwrap_or_default(),
        },
        _ => Sheet::Strip {
            frame_px: jnum(&d, "frame_px").unwrap_or(0.0) as f32,
        },
    };
    let clips = d
        .get("clips")
        ...
        .map(|arr| {
            arr.iter_shared()
                .filter_map(|c| c.try_to::<VarDictionary>().ok())
...
pub(crate) fn parse_clip(d: VarDictionary) -> Option<Clip> {
    ...
    let files = d
        ...
        .map(|arr| {
            arr.iter_shared()
                .filter_map(|f| f.try_to::<GString>().ok().map(|g| g.to_string()))
                .collect()
        })
...
/// Read a string field from a Json-parsed VarDictionary.
pub(crate) fn jstr(d: &VarDictionary, key: &str) -> Option<String> {
    d.get(key)
...
pub(crate) fn jnum(d: &VarDictionary, key: &str) -> Option<f64> {
