...
`<clip>_strip<N>.png` files under `app/godot/assets/<name>/`, and appends one roster entry
per new pack. Existing characters are preserved; duplicate names are rejected.

Install Pillow explicitly in the selected Python environment before running conversion.
"""
...
ALIAS = {
    ...
    "attack": "jab", "jab1": "jab", "nattack": "jab",
    "kick": "ftilt",  # Falcon's rip names its forward tilt "kick"
...
def ensure_pillow() -> None:
    try:
        ...
        pass
    raise SystemExit("Pillow is required: install tools/requirements.txt in tools/.venv")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import new sprite packs while preserving the roster")
    parser.add_argument("--manifest", type=Path, default=MANIFEST)
    parser.add_argument("--assets", type=Path, default=ASSETS)
    args = parser.parse_args()
    ensure_pillow()
    installer = importlib.import_module("0_import_character")
    packs = tomllib.loads(args.manifest.read_text()).get("pack", [])
    for pack in packs:
        installer.relative_name(pack["name"])
        files = source_files(pack)
        if not files:
            raise SystemExit(f"No PNG files for {pack['name']}")
        with tempfile.TemporaryDirectory(prefix="kneeman-pack-") as scratch:
            output = Path(scratch)
            builder = build_grid_pack if pack.get("kind") == "grid" else build_strip_pack
            entry = builder(pack, files, output)
            frames = {p.name: p.read_bytes() for p in output.glob("*.png")}
            destination = installer.install(entry, frames, assets=args.assets)
            print(f"Imported {len(entry['clips'])} clips -> {destination}")
    print("Run just import, then launch; game3-publish rebuilds the production assets.")
