...
pub enum Signal {
    ...
    Room { code: String },
    /// Reconnect resume: the (new) host ships the authoritative sim snapshot to rebuild from. Boxed
    /// so every `Signal`/`Event`/`Effect` temporary isn't sized to the ~26KB `SimState` (see the same
    /// note on `Event::ChannelClosed`).
    Resume { seed: Box<SimState> },
...
pub enum Event {
    ...
    /// session) so the reconnect can resume from it, plus the clock for the reconnect deadline. Boxed:
    /// `SimState` is Copy but ~26KB, and Rust enums are max-variant-sized, so an unboxed field here
    /// would make every `Event` (including e.g. plain `Tick`) that size on the stack.
    ChannelClosed {
        last_state: Box<SimState>,
        now_ms: u64,
    },
...
pub enum Effect {
    ...
    /// `seed` is boxed for the same reason as `Event::ChannelClosed`: keep `Effect` pointer-sized.
    BeginSession {
        seed: Box<SimState>,
        tune: Tune,
        role: Role,
...
pub struct Lobby {
    ...
    pub tune: Tune,
    /// The snapshot to resume a rebuilt session from (reconnect only); `None` = spawn fresh. Boxed so
    /// `Lobby` itself (and every `Signal`/`Event`/`Effect` built from it) stays off the ~26KB
    /// `SimState` size; see `Event::ChannelClosed` for the full rationale.
    pub resume: Option<Box<SimState>>,
    pub got_tune: bool,
    pub got_resume: bool,
...
impl Lobby {
    ...
    pub fn reduce(&mut self, ev: Event, out: &mut Vec<Effect>) {
        match ev {
            Event::Matchmake { room } => {
                ...
                self.room = room.clone().map(|code| Room {
                    code,
                    deadline_ms: None,
                });
                self.resume = None;
            ...
            Event::ChannelClosed { last_state, now_ms } => {
                self.begin_reconnect(last_state, now_ms, out)
            }
            Event::Tick { now_ms } => {
    ...
    fn on_signal(&mut self, sig: Signal, out: &mut Vec<Effect>) {
        match sig {
            ...
            Signal::Answer { .. } => out.push(Effect::SetRemote {
                kind: SdpKind::Answer,
            }),
    ...
    fn setup_peer(&mut self, role: Role, out: &mut Vec<Effect>) {
        ...
        if role == Role::Host {
            ...
            } else if let Some(seed) = self.resume.clone() {
            ...
            out.push(Effect::Send(Signal::Tune {
    ...
    fn begin_reconnect(&mut self, last_state: Box<SimState>, now_ms: u64, out: &mut Vec<Effect>) {
        ...
        out.push(Effect::Dial {
            room: Some(code.clone()),
        });
        self.room = Some(Room {
            code,
            deadline_ms: Some(now_ms + RECONNECT_WINDOW_MS),
        });
