...
use crate::v1::world::{Envelope, PlayerId, Schema, WorldEvent};
...
fn upcast_v0(old: WorldEventV0) -> WorldEvent {
    match old {
        ...
        WorldEventV0::PlacePlatform { at, len, owner: _ } => WorldEvent::PlacePlatform {
            at,
            len,
...
pub fn decode(env: &Envelope) -> WorldEvent {
    match env.schema {
        ...
        Schema(s) => {
            panic!("unknown event schema {s}: binary too old (should have refused at handshake)")
        }
    }
}
...
pub fn encode(ev: &WorldEvent) -> Envelope {
    Envelope {
        schema: Schema(1),
        payload: crate::v1::world::canon(ev),
    }
}
...
mod tests {
    use super::*;
    use crate::v1::world::{EventId, WorldEvent};

    #[test]
    fn old_schema_upcasts_to_current() {
        // a row written by an old build (schema 0, no class/stroke fields).
        let old = WorldEventV0::PlacePlatform {
            at: Vector2::new(5.0, 6.0),
            len: 20.0,
            owner: 2,
        };
        let env = Envelope {
    ...
    fn current_schema_roundtrips() {
        let ev = WorldEvent::ErasePlatform {
            placed: EventId([7u8; 32]),
        };
