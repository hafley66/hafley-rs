...
use crate::v1::{SegClass, StrokeId, Tune, Vector2};
...
// Content-addressed asset identity + hashing moved to the shared shell module `crate::asset_wire`
// (the V4 path uses them too); re-exported here so every v1 caller keeps its path.
pub use crate::asset_wire::{ASSET_MAX, AssetId, asset_id};
...
/// Why `put_asset` refused a blob. In-process only (never wired/hashed), so it carries none of the
/// `WorldEvent` byte-stability discipline (§2).
#[derive(Copy, Clone, PartialEq, Eq, Debug)]
pub enum AssetErr {
    /// `len` exceeded `ASSET_MAX`. The store must check before writing a single byte — no partial write.
...
mod tests {
    ...
    fn event_id_chains_and_dedupes() {
        let payload = canon(&WorldEvent::SetRule {
            key: RuleKey(3),
            val: RuleVal(1.5),
        });
    ...
    fn frozen_events() -> Vec<WorldEvent> {
        vec![
            WorldEvent::PlacePlatform {
                at: Vector2::new(1.0, 2.0),
                len: 32.0,
                class: SegClass::Floor,
                stroke: 0,
                owner: PlayerId::WORLD,
            },
            WorldEvent::ErasePlatform {
                placed: EventId([1u8; 32]),
            },
            WorldEvent::Revert {
                target: EventId([2u8; 32]),
            },
            WorldEvent::SetRule {
                key: RuleKey(3),
                val: RuleVal(1.5),
            ...
            WorldEvent::PlayerJoin {
                player: PlayerId([4u8; 32]),
                name: "ann".to_string(),
                color: 0x11223344,
                char_pick: 5,
            },
            WorldEvent::PlayerLeave {
                player: PlayerId([4u8; 32]),
            },
            WorldEvent::SetBackground {
                bg: Some(AssetId([5u8; 32])),
            },
