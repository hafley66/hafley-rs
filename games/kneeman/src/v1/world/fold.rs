...
use crate::v1::world::{
    AssetId, BuildVersion, EventId, Node, PlayerId, Schema, Seq, WorldEvent, canon, event_id,
};
...
impl Ingestor {
    pub fn ingest(&mut self, incoming: &[Node]) {
        let known: HashSet<EventId> = self
            .chain
            .iter()
...
mod tests {
    use super::*;
    use crate::v1::world::{RuleKey, RuleVal};
    ...
    fn upgrade_moves_the_version() {
        let nodes = chain(&[WorldEvent::Upgrade {
            to: BuildVersion(7),
            mig: crate::v1::world::MigrationId(1),
        }]);
    ...
    fn player_join_then_leave_keeps_entity_and_owned_ink() {
        ...
        let p = w
            .players
            .get(&pid)
            .expect("player still an entity after leaving");
    ...
    fn set_background_folds_last_write_wins() {
        ...
        let cleared = chain(&[
            WorldEvent::SetBackground { bg: Some(a) },
            WorldEvent::SetBackground { bg: None },
        ]);
    ...
    fn relation_classifies_ancestor_and_fork() {
        use crate::v1::world::{Schema, event_id};
        ...
        let mk = |x: f32| {
            let ev = place(x);
            let id = event_id(Some(base[1].id), Schema(1), &crate::v1::world::canon(&ev));
            Node {
                id,
                parent: Some(base[1].id),
                seq: crate::v1::world::Seq(2),
                ev,
            }
        };
