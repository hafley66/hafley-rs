...
use crate::v1::world::{
    ASSET_MAX, AssetErr, AssetId, EventId, Node, Schema, Seed, Seq, WorldEvent, WorldId, asset_id,
    canon, event_id, world_id,
};
...
pub trait WorldStore {
    ...
    /// Store bytes under their own hash; idempotent (same bytes twice -> same id, one stored copy).
    /// Rejects `bytes.len() > ASSET_MAX` with no partial write. Returns the `AssetId` a `SetBackground`
    /// records (plans/world-protocol.md §T, task W1).
    fn put_asset(&mut self, bytes: &[u8]) -> Result<AssetId, AssetErr>;
    /// Fetch by id, recomputing `blake3` on the way out (self-verifying: the id IS the content hash).
    /// `None` covers both "not held" and "held but tampered" — a mismatch never surfaces wrong bytes.
    fn get_asset(&self, id: AssetId) -> Option<Vec<u8>>;

    /// Every `AssetId` this world's `Seed.bg` and folded `SetBackground` events reference that isn't
    /// held locally (§T task W1.6). Default, backend-agnostic: built from `seed`/`since`/`get_asset`
    /// alone, so it needs no override. Order: first-referenced — `Seed.bg` (if any), then each
    /// `SetBackground` in log order — deduped, dropping ids already held.
    fn missing_assets(&self, id: WorldId) -> Vec<AssetId> {
        let mut seen = std::collections::HashSet::new();
        let mut refs = Vec::new();
        if let Some(seed) = self.seed(id) {
            if let Some(a) = seed.bg {
                if seen.insert(a) {
                    refs.push(a);
                }
            }
        }
        for node in self.since(id, None) {
            if let WorldEvent::SetBackground { bg: Some(a) } = node.ev {
                if seen.insert(a) {
                    refs.push(a);
                }
            }
        ...
        refs.into_iter()
            .filter(|a| self.get_asset(*a).is_none())
            .collect()
    }
...
impl WorldStore for MemStore {
    ...
    fn has(&self, id: WorldId, ev: EventId) -> bool {
        self.logs
            .get(&id)
            .is_some_and(|l| l.iter().any(|n| n.id == ev))
    }
    fn put_snapshot(&mut self, id: WorldId, upto: EventId, blob: &[u8]) {
    ...
    fn get_asset(&self, id: AssetId) -> Option<Vec<u8>> {
        self.assets
            .get(&id)
            .cloned()
            .filter(|bytes| asset_id(bytes) == id) // tamper-evident: recheck the hash on read
    }
    fn put_slot(&mut self, id: WorldId, slot: &Slot) {
    ...
    fn cache_bytes(&self, id: WorldId) -> u64 {
        let log: usize = self
            .logs
            .get(&id)
            .map_or(0, |l| l.iter().map(|n| canon(&n.ev).len()).sum());
...
impl WorldStore for SqliteStore {
    ...
    fn seed(&self, id: WorldId) -> Option<Seed> {
        self.conn
            .query_row("SELECT seed FROM world WHERE id=?1", [&id.0[..]], |r| {
                r.get::<_, Vec<u8>>(0)
            })
            .ok()
    ...
    fn append(&mut self, id: WorldId, ev: &WorldEvent) -> EventId {
        ...
        let seq: i64 = self
            .conn
            .query_row(
                "SELECT COUNT(*) FROM world_event WHERE world_id=?1",
                [&id.0[..]],
                |r| r.get(0),
            )
            .unwrap_or(0);
    ...
    fn head(&self, id: WorldId) -> Option<EventId> {
        self.conn
            .query_row(
                "SELECT head FROM world_head WHERE world_id=?1",
                [&id.0[..]],
                |r| r.get::<_, Vec<u8>>(0),
            )
            .ok()
    ...
    fn since(&self, id: WorldId, from: Option<EventId>) -> Vec<Node> {
        let mut stmt = self
            .conn
            .prepare(
                "SELECT id, parent, seq, payload FROM world_event WHERE world_id=?1 ORDER BY seq",
            )
    ...
    fn get_snapshot(&self, id: WorldId) -> Option<(EventId, Vec<u8>)> {
        self.conn
            .query_row(
                "SELECT upto, blob FROM world_snapshot WHERE world_id=?1",
                [&id.0[..]],
                |r| {
                    Ok((
                        EventId(to32(r.get::<_, Vec<u8>>(0)?)),
                        r.get::<_, Vec<u8>>(1)?,
                    ))
                },
            )
    ...
    fn put_asset(&mut self, bytes: &[u8]) -> Result<AssetId, AssetErr> {
        ...
        self.conn
            .execute(
                "INSERT OR IGNORE INTO asset(id, bytes) VALUES(?1, ?2)",
    ...
    fn get_asset(&self, id: AssetId) -> Option<Vec<u8>> {
        self.conn
            .query_row("SELECT bytes FROM asset WHERE id=?1", [&id.0[..]], |r| {
                r.get::<_, Vec<u8>>(0)
            })
            .ok()
            .filter(|bytes| asset_id(bytes) == id) // tamper-evident: recheck the hash on read
    ...
    fn slots(&self, id: WorldId) -> Vec<Slot> {
        let mut stmt = self
            .conn
            .prepare(
                "SELECT label, at_ms, head, seq FROM world_slot WHERE world_id=?1 ORDER BY at_ms",
            )
    ...
    fn del_slot(&mut self, id: WorldId, label: &str) {
        self.conn
            .execute(
                "DELETE FROM world_slot WHERE world_id=?1 AND label=?2",
                rusqlite::params![&id.0[..], label],
            )
            .expect("del_slot");
    }
    fn cache_bytes(&self, id: WorldId) -> u64 {
        let log: i64 = self
            .conn
            .query_row(
                "SELECT COALESCE(SUM(LENGTH(payload)),0) FROM world_event WHERE world_id=?1",
                [&id.0[..]],
                |r| r.get(0),
            )
            .unwrap_or(0);
        let snap: i64 = self
            .conn
            .query_row(
                "SELECT COALESCE(LENGTH(blob),0) FROM world_snapshot WHERE world_id=?1",
                [&id.0[..]],
                |r| r.get(0),
            )
            .unwrap_or(0);
        let assets: i64 = self
            .conn
            .query_row(
                "SELECT COALESCE(SUM(LENGTH(bytes)),0) FROM asset",
                [],
                |r| r.get(0),
            )
            .unwrap_or(0);
...
mod tests {
    use super::*;
    use crate::v1::world::fold::fold;
    use crate::v1::world::{BuildVersion, RuleKey, RuleVal, StageId};
    ...
    fn asset_blob_roundtrips_by_content_hash() {
        ...
        let id = s.put_asset(gif).unwrap();
        assert_eq!(s.put_asset(gif).unwrap(), id); // same bytes -> same id (idempotent put)
        assert_eq!(s.get_asset(id).as_deref(), Some(&gif[..]));
        assert_eq!(s.get_asset(crate::v1::world::AssetId([0u8; 32])), None); // unknown id -> miss
    }
    ...
    fn compaction_preserves_state_through_the_store() {
        use crate::v1::world::WorldEvent;
        let mut s = SqliteStore::in_memory().unwrap();
        let id = s.publish(&seed());
        let evs: Vec<WorldEvent> = (0..5)
            .map(|k| WorldEvent::SetRule {
                key: RuleKey(k),
                val: RuleVal(k as f32),
            })
            .collect();
        for ev in &evs {
            s.append(id, ev);
        ...
        assert_eq!(upto, cut);
        let base: crate::v1::world::fold::World = bincode::deserialize(&blob).unwrap();
        let rebuilt = crate::v1::world::fold::fold_from(&base, &s.since(id, None)); // snapshot + tail
    ...
    // ─────────────────────────────────────────────────────────────────────────────────────────────
    // Asset blob contract (plans/world-protocol.md §T, task W1, items 1-6). Each item is one #[test];
    // the shared assertions live in a generic `check<S: WorldStore>` fn run against MemStore and
    // SqliteStore inline (the shared-test-fn pattern), so both backends stay observationally identical.
    // ─────────────────────────────────────────────────────────────────────────────────────────────
    ...
    // 1. put_asset returns asset_id(bytes); get_asset round-trips.
    #[test]
    fn put_asset_returns_hash_and_get_roundtrips() {
        fn check<S: WorldStore>(mut s: S) {
            let bytes = b"asset payload bytes".as_slice();
            let id = s.put_asset(bytes).expect("under ASSET_MAX");
            assert_eq!(id, asset_id(bytes)); // put's id IS the content hash
            assert_eq!(s.get_asset(id).as_deref(), Some(bytes));
        }
        check(MemStore::new());
        check(SqliteStore::in_memory().unwrap());
    }

    // 2. put is idempotent: same bytes twice -> same id, one stored copy (sqlite: one row).
    #[test]
    fn put_asset_is_idempotent() {
        fn check<S: WorldStore>(mut s: S) {
            let bytes = b"same bytes, put twice".as_slice();
            let a = s.put_asset(bytes).unwrap();
            let b = s.put_asset(bytes).unwrap();
            assert_eq!(a, b);
        }
        check(MemStore::new());
        check(SqliteStore::in_memory().unwrap());

        // sqlite-specific: one row, not two.
        let mut s = SqliteStore::in_memory().unwrap();
        let bytes = b"row count check".as_slice();
        s.put_asset(bytes).unwrap();
        s.put_asset(bytes).unwrap();
        let rows: i64 = s
            .conn
    ...
    // 3. get_asset on an unknown id -> None.
    #[test]
    fn get_asset_of_unknown_id_is_none() {
        fn check<S: WorldStore>(s: S) {
            assert_eq!(s.get_asset(AssetId([9u8; 32])), None);
        }
        check(MemStore::new());
        check(SqliteStore::in_memory().unwrap());
    }

    // 4. put_asset of bytes > ASSET_MAX -> Err, and nothing was written (no partial write).
    #[test]
    fn put_asset_over_max_is_rejected_with_no_partial_write() {
        fn check<S: WorldStore>(mut s: S) {
            let bytes = vec![7u8; ASSET_MAX + 1];
            let err = s.put_asset(&bytes).unwrap_err();
            assert_eq!(
                err,
                AssetErr::TooLarge {
                    len: bytes.len(),
            ...
            assert_eq!(s.get_asset(asset_id(&bytes)), None); // rejected -> never stored
        }
        check(MemStore::new());
        check(SqliteStore::in_memory().unwrap());

        // sqlite-specific: the asset table stays empty, not just the id lookup.
        let mut s = SqliteStore::in_memory().unwrap();
        let _ = s.put_asset(&vec![7u8; ASSET_MAX + 1]);
        let rows: i64 = s
            .conn
            .query_row("SELECT COUNT(*) FROM asset", [], |r| r.get(0))
            .unwrap();
    ...
    // 5. self-verification (sqlite-specific): a tampered row fails get_asset's hash recheck -> None,
    // never the wrong bytes.
    #[test]
    fn tampered_asset_bytes_fail_the_hash_recheck() {
        let mut s = SqliteStore::in_memory().unwrap();
        let bytes = b"pristine bytes".as_slice();
        let id = s.put_asset(bytes).unwrap();
        s.conn
            .execute(
                "UPDATE asset SET bytes=?1 WHERE id=?2",
                rusqlite::params![b"corrupted!!!".as_slice(), &id.0[..]],
            )
            .unwrap();
        assert_eq!(s.get_asset(id), None); // tamper-evident: recompute fails -> None, not wrong bytes
    }

    // 6. missing_assets walks Seed.bg + folded SetBackground events, deduped, first-referenced order.
    #[test]
    fn missing_assets_walks_seed_and_log() {
        fn check<S: WorldStore>(mut s: S) {
            let bytes_a = b"background a".as_slice();
            let bytes_b = b"background b".as_slice();
            let a = s.put_asset(bytes_a).unwrap(); // store holds only a
            let b = asset_id(bytes_b); // b is referenced but never put

            let id = s.publish(&seed_with_bg(Some(a)));
            s.append(id, &WorldEvent::SetBackground { bg: Some(b) });

            assert_eq!(s.missing_assets(id), vec![b]); // a is held, b is not
            s.put_asset(bytes_b).unwrap();
            assert_eq!(s.missing_assets(id), Vec::<AssetId>::new()); // now both are held
        }
        check(MemStore::new());
        check(SqliteStore::in_memory().unwrap());
    }
