...
| app | `smash/` | Pigeon behavior/content selection, Small Battlefield setup, game-specific control policy, match composition and integration scenarios |
| mixed | Both, explicitly separated | Bone lab: reusable attachment evaluation plus Pigeon fixtures; ship lab: reusable occupancy/control machinery plus ship definition |
...
| --- | --- | --- |
| m11 | [Permanent coverage join](classification/17_coverage.txt); authored profile `classification/13_port.tsp`; generated observations `classification/14_runtime_inventory.json` and `classification/16_coverage.json` | Pigeon/Falcon-like gameplay profile maps 22 source states to executable Rust phases and excludes 93 item-system rows with expanded reasons. Current source inventory has 341 states, 1,314 callback associations across 721 callbacks, 1,993 direct calls and 1,939 unresolved records. Exact axes: state mapping 22/248 (8.87%), transition/callback mapping 0/1,314 (0%), ordered guard qualification 0/1,993 (0%), live reachability 18/248 (7.26%), rollback 0/248 (0%), source fidelity 0/248 (0%), strict intersection 0/248 (0%). Existing prove receipt is legacy context only and requirement receipt count is 0, so it contributes zero. Missing source mappings and callback/call qualifications stay unresolved. |
...
| m1.hit | [Launch proof](blender-godot-sqlite-proof/pigeon-lab/20_launch_readme.md), [sandbag](blender-godot-sqlite-proof/simulation-core/1_sandbag.rs) | Parry intersection, ssbm_utils formula calls, Rapier target flight. Weight-100 target and Melee formulas need explicit PM qualification. |
| m1.output / m2.boundary | [SQL proof](blender-godot-sqlite-proof/pigeon-lab/26_sql_readme.md), [Godot proof](blender-godot-sqlite-proof/pigeon-lab/33_godot_readme.md), [faults](blender-godot-sqlite-proof/pigeon-lab/63_fault_readme.md), `pigeon-lab/contracts/0_presentation.tsp` | Existing row/payload/codegen and adapter proofs; reusable extraction remains. |
| m2.state / m2.rollback / m2.capture | [Shared provenance](shared/0_reuse.md), [latest lab receipt](blender-godot-sqlite-proof/pigeon-lab/109_tasks.md) | Shared reducers, statig buffer, GGRS proof harness, capture. Historical scoped tests/MP4 and WASM compile pass; full cross-target match is unverified. |
...
| m9.translator | `crates/content/src/5_source.rs`; `tree-sitter` 0.24.7 and `tree-sitter-c` 0.23.4 (MIT) Cargo pins | Bounded lowering over the existing `game-content` `ingest` parser. Expression nodes: parentheses, unary `-`, binary `*`, the six comparisons, conditionals, `fp->input.lstick[0].x`, `fp->facing_dir`, `fp->co_attrs.*`, `p_ftCommonData->*`, `ftCo_MS_*`, `Ft_MF_*`, number/bool literals and locals. Statement nodes: scalar/`Vec3` assignment, `fp->cmd_vars[i]` and `fp->x2227_b0` writes and a fixed call table. Anything else returns `SourceError::UnsupportedExpression`. No new parser or general C compiler. |
| m9.port | Generated `crates/ftcommon/src/generated/0_ftcommon.rs`; boundary `crates/ftcommon/src/0_types.rs`; tests `crates/ftcommon/tests/1_source_rules.rs` | `game-ftcommon` owns `FighterQuery`, `CommonData`, `CoAttrs`, `Vec3`, `FtMotionId`, `MotionFlags` and the `FtCommonEffect` enum. `ftCo_800C97A8` stays a pure boolean guard; `ftCo_Jump_Enter` returns `[FtCommonEffect; 4]` and `ftCo_JumpAerial_Enter_Basic` returns `[FtCommonEffect; 3]`, each covering every source call and write in order with repository/revision/path/line-span, the C names and the source guard/binding text. Six fixtures assert ordered effects, computed aerial velocity from typed `co_attrs`, the facing sign flip and a non-zero `x78`. |
| m9.open | Generated provenance comments; `4_tasks.md` S11 | `p_ftCommonData->x34`/`x78` and `fp->co_attrs` remain typed inputs, never literals. Effects whose semantics are not modeled keep the decomp symbol (`ftCommon_8007D5D4`, `FtCo_800CB110`, `FtCo_800CBAC4`); `Fighter_ChangeMotionState` is typed. Nothing in the live simulation consumes `game-ftcommon` yet. Stage 2.7 on the draft/testing exit, no promotion. |
...
No automatic claim that every Pigeon special is available.
