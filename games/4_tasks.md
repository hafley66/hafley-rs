...
| S11 | In progress, generated port | Bounded C-to-Rust generation for `ftCo_800C97A8`, `ftCo_Jump_Enter` and `ftCo_JumpAerial_Enter_Basic` from pinned `games/vendor/melee`; generated `crates/ftcommon` retains repository/revision/path/line-span, keeps the C names and lowers `p_ftCommonData->x34`/`x78` to typed inputs. Fixtures compare generated decisions against `smash/.../2_source_rules.json`. Terminal: `just source-rules-check` stale-checks the generated file and `cargo test -p game-ftcommon` passes. Untranslated engine calls and a live consumer remain open. |
