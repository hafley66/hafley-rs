...
use base64::Engine;
use game_content::{bake, decode_file, decode_html, emit_chart, TransitionSpec, Trigger};
use std::path::Path;
...
#[test]
fn chart_generation_resolves_names_and_rejects_invalid_identity() {
    let mut actions = [
        decode_file(&fixture("4_pm36_Wait1.html")).unwrap(),
        decode_file(&fixture("6_pm36_JumpSquat.html")).unwrap(),
    ];
    ...
    assert_eq!(emit_chart(&actions, &spec).unwrap(), concat!(
        "// Generated by smash-import. Do not edit.\n\n",
        "use game_content::{Transition, Trigger};\n\n",
        "pub const TRANSITIONS: [Transition; 1] = [\n",
        "    Transition { from: 0, trigger: Trigger::JumpPress, to: 1 },\n",
        "];\n",
    ));
    assert_eq!(emit_chart(&actions, &[TransitionSpec { to: "Missing", ..spec[0] }]).unwrap_err(),
        "missing action Missing");
    actions[1].name = "Wait1".into();
    assert_eq!(emit_chart(&actions, &spec).unwrap_err(), "duplicate action Wait1");
}
