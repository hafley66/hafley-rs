...
together; this document retains their provenance. The Pigeon lab consumes those
...
- `rollback`: existing RollbackSim/Game/GgrsConfig and sync-test construction.
  Shared apply_request executes save/load/advance for both Game and the Pigeon
  lab's instrumented request observer. Presentation stays outside World.

The Pigeon rollback golden and fault tests must remain green after changing these
...
Current user scope: site-wide acquisition is deferred in Pigeon `104_tasks.md`; import
Pigeon's available source data with unsupported common callbacks reported; model
...
- `input`: macro-free statig 0.4.1 lane buffer. `Buffer::advance(press, eligible,
  ...
  is inert. Pigeon's existing Redux Step consumes this implementation.
  `PlayerInput` and quantization were ported from `crates/input` at `8646fa2`;
  the Pigeon adapter still accepts its existing button bits. Four-axis controller
  integration and V1 tech/coyote buffers remain pending.
- `capture`: existing Pigeon wgpu/FFmpeg recorder and shader moved here.
  ...
  allocation is unchanged. Pigeon pixel assertions remain in `1_gpu.rs`;
...
- `rollback::proof::run<S>` accepts borrowed configuration and a tick/player
  ...
  The independent fixture uses two counters; Pigeon consumes the same harness.
- `capture::text` is the existing font8x8 mesh renderer moved from Pigeon.
  ...
  owns GPU/encoder resources. Existing Pigeon text callers re-export this code.

Pigeon's `just held-proof` records both old and new scenarios in
